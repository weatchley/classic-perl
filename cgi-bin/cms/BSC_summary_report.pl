...
#
# BSC Summary Report format for CMS
#
# $Source: /data/dev/rcs/cms/perl/RCS/BSC_summary_report.pl,v $
# $Revision: 1.2 $
# $Date: 2002/02/21 22:30:02 $
# $Author: naydenoa $
# $Locker:  $
# $Log: BSC_summary_report.pl,v $
# Revision 1.2  2002/02/21 22:30:02  naydenoa
# Allowed display of commitments without assigned managers and leads
#
# Revision 1.1  2001/12/18 21:20:56  naydenoa
# Initial revision
#
#
...
<script language="JavaScript" type="text/javascript">
<!--
    doSetTextImageLabel('BSC Manager Summary Report');
//-->
</script>
</head>
pageheader
...
<form name=reportfilter method=post action="$ONCSCGIDir/BSC_summary_report.pl">
...
#    $orderby = 'ORDER BY u.lastname, u.firstname, c.description, com.commitmentid';
    $leadsort='leaddesc';
}
if ($sortorder eq 'leaddesc') {
    $orderby = 'ORDER BY com.lleadid DESC, c.description, com.commitmentid';
#    $orderby = 'ORDER BY u.lastname DESC, u.firstname desc, c.description, com.commitmentid';
    $leadsort='lead';
}
...
#    $orderby = 'order by m.lastname, m.firstname, com.issueid, com.commitmentid'; 
    $cmgrsort='cmgrdesc';
}
if ($sortorder eq 'cmgrdesc') {
    $orderby = 'order by com.managerid desc, com.issueid, com.commitmentid'; 
#    $orderby = 'order by m.lastname desc, m.firstname, com.issueid, com.commitmentid'; 
    $cmgrsort='cmgr';
}
...
########################
print "<table width=900 border=1 cellpadding=2 cellspacing=0 bgcolor=#ffffff>\n";
print "<tr bgcolor=#ffefad><td colspan=16><b>BSC Senior Manager/Project Manager Summary Report</b></td></tr>\n";
...
print "<td nowrap><font size=2><b><a href=\"javascript:submitsort('$cmgrsort');\">RM</a>/<a href=\"javascript:submitsort('$leadsort');\">LL</a></td>\n";
...
eval {
    ...
    while (my ($category, $iid, $itext, $accnum, $cid, $ctext, $extid, $rmid, $llid, $fulfil, $due, $stat) = $csr -> fetchrow_array) {
	
	$extid = "&nbsp;" if (!$extid);
	$fulfil = "&nbsp;" if (!$fulfil);
	print "<tr valign=top><td><font size=2>$category</td>\n";
	print "<td><font size=2><a href=\"javascript:DisplayIssue($iid);\">" . formatID2($iid, 'I') . "</a></td>\n";
	print "<td><font size=2>" . getDisplayString($itext, 60) . "</td>\n";
	print "<td><font size=2>$accnum&nbsp;</td>";
	if ($cid) {
	    print "<td><font size=2><a href=\"javascript:DisplayCommitment($cid);\">" . formatID2($cid, 'C') . "</a></td>\n";
	    print "<td><font size=2>" . getDisplayString ($ctext, 60) . "</td>\n";
	    print "<td><font size=2>$extid&nbsp;</td>\n";
	    ...
	    print "<td><font size=2>$cmgrfirst&nbsp;$cmgrlast<br>$cleadfirst&nbsp;$cleadlast</td>\n";
	    print "<td><font size=2>$fulfil&nbsp;</td>\n";
	    print "<td><font size=2>$due&nbsp;</td>\n";
	    print "<td><font size=2>$stat</td>\n";
	}
	else {
	    print "<td colspan=12><fint size=2>There are no commitments for this issue</td></tr>\n";
	}
    }
    print "</table>\n";
    $csr->finish;
