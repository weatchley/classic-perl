...
# Text Menus file
#
# $Source: /home/atchleyb/rcs/dms/perl/RCS/Text_Menus.pm,v $
#
# $Revision: 1.1 $
#
# $Date: 2002/03/08 21:13:35 $
#
# $Author: atchleyb $
#
# $Locker:  $
#
# $Log: Text_Menus.pm,v $
# Revision 1.1  2002/03/08 21:13:35  atchleyb
# Initial revision
#
#
#
...
# Table Menu
        ...
        for (my $i = 0; $i <= $#{ $self->{menuList} }; $i++) {
            ...
            if (substr($self->{menuList}[$i]{contents},0,11) ne 'javascript:') {
                $outputstring .= "<td align=center vaglin=bottom><a href=\"javascript:showHide" . $menuName . "Menu(" . $name . "MenuOutline);\" style=\"text-decoration:none\"><b>" . $label . "</b></a></td>\n";
            } else {
                $outputstring .= "<td align=center vaglin=bottom><a href=\"javascript:showHide" . $menuName . "Menu(" . $name . "MenuOutline);" . substr($self->{menuList}[$i]{contents},11) . "\" style=\"text-decoration:none\"><b>" . $label . "</b></a></td>\n";
            }
        }
...
# Tabs Menu
        ...
        for (my $i = 0; $i <= $#{ $self->{menuList} }; $i++) {
            ...
            if ($self->{imageSource} gt ' ') {
                if (substr($self->{menuList}[$i]{contents},0,11) ne 'javascript:') {
                    $outputstring .= "<td align=center vaglin=bottom><table cellpadding=0 cellspacing=0 border=0><tr><td width=7><img src='" . $self->{imageSource} . $self->{tabLeftLight} . "'></td><td bgcolor=" . $self->{'tabLightColor'} . " width=136 align=center><a href=\"javascript:showHide" . $menuName . "Menu(" . $name . "MenuOutline,'F');//showHide" . $menuName . "TabUnderline(" . $name ."tabbottom);\" id=\"$name" . "Link\" style=\"text-decoration:none\"><b><font size=-1>" . $label . "</font></b></a></td><td width=7><img src='" . $self->{imageSource} . $self->{tabRightLight} . "'></td></tr><tr><td colspan=3><img src='" . $self->{imageSource} . $self->{tabBottom} . "' id='" . $name ."TabBottom' name='" . $name ."TabBottom'></td></tr></table></td>\n";
                } else {
                    $outputstring .= "<td align=center vaglin=bottom><table cellpadding=0 cellspacing=0 border=0><tr><td width=7><img src='" . $self->{imageSource} . $self->{tabLeftLight} . "'></td><td bgcolor=" . $self->{'tabLightColor'} . " width=136 align=center><a href=\"javascript:showHide" . $menuName . "Menu(" . $name . "MenuOutline,'F');//showHide" . $menuName . "TabUnderline(" . $name ."tabbottom);" . substr($self->{menuList}[$i]{contents},11) . "\" id=\"$name" . "Link\" style=\"text-decoration:none\"><b><font size=-1>" . $label . "</font></b></a></td><td width=7><img src='" . $self->{imageSource} . $self->{tabRightLight} . "'></td></tr><tr><td colspan=3><img src='" . $self->{imageSource} . $self->{tabBottom} . "' id='" . $name ."TabBottom' name='" . $name ."TabBottom'></td></tr></table></td>\n";
                }
            } else {
                if (substr($self->{menuList}[$i]{contents},0,11) ne 'javascript:') {
                    $outputstring .= "<td align=center><a href=\"javascript:showHide" . $menuName . "Menu(" . $name . "MenuOutline);\" id=\"$name" . "Link\" style=\"text-decoration:none\"><b>" . $label . "</b></a></td>\n";
                } else {
                    $outputstring .= "<td align=center><a href=\"javascript:showHide" . $menuName . "Menu(" . $name . "MenuOutline);" . substr($self->{menuList}[$i]{contents},11) . "\" id=\"$name" . "Link\" style=\"text-decoration:none\"><b>" . $label . "</b></a></td>\n";
                }
                if ($i != $last) {
                    $outputstring .= "<td>";
...
# List Menu
        ...
        for (my $i = 0; $i <= $last; $i++) {
            ...
            $label =~ s/ /&nbsp;/g;
            if (substr($self->{menuList}[$i]{contents},0,11) ne 'javascript:') {
                $outputstring .= "<a href=\"javascript:showHide" . $menuName . "Menu(" . $name . "MenuOutline);\" style=\"text-decoration:none\"><b>" . $label . "</b></a>\n";
            } else {
                $outputstring .= "<a href=\"javascript:showHide" . $menuName . "Menu(" . $name . "MenuOutline);" . substr($self->{menuList}[$i]{contents},11) . "\" style=\"text-decoration:none\"><b>" . $label . "</b></a>\n";
            }
            if ($i != $last) {
                foreach my $j (1 .. 10) {$outputstring .= "&nbsp;";}
            }
...
# Bullets Menu
        ...
        for (my $i = 0; $i <= $#{ $self->{menuList} }; $i++) {
            ...
            if (substr($self->{menuList}[$i]{contents},0,11) ne 'javascript:') {
                $outputstring .= "<li><a href=\"javascript:showHide" . $menuName . "Menu(" . $name . "MenuOutline);\">" . $self->{menuList}[$i]{label} . "</a><br>\n";
            } else {
                $outputstring .= "<li><a href=\"javascript:showHide" . $menuName . "Menu(" . $name . "MenuOutline);" . substr($self->{menuList}[$i]{contents},11) . "\">" . $self->{menuList}[$i]{label} . "</a><br>\n";
            }
        }
