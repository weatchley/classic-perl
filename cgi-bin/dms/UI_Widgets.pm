# Library of UI widget routines for cgi scripts (used first for DMS)

#
# $Source: /data/dev/rcs/dms/perl/RCS/UI_Widgets.pm,v $
#
# $Revision: 1.3 $
#
# $Date: 2002/06/03 22:24:59 $
#
# $Author: munroeb $
#
# $Locker:  $
#
# $Log: UI_Widgets.pm,v $
# Revision 1.3  2002/06/03 22:24:59  munroeb
# fixed spelling on getFiscalYear subroutine
#
# Revision 1.2  2002/06/03 20:58:29  munroeb
# added getFiscalYear function
#
# Revision 1.1  2002/03/08 21:14:37  atchleyb
# Initial revision
#
#
#
...
@EXPORT = qw (
   ...
   &BuildPrintCommentResponse &BuildPrintSummaryCommentResponse &doAlertBox &getFiscalYear
);
@EXPORT_OK = qw(
   &build_dual_select &build_select_from_piclist &build_drop_box &build_date_selection &build_radio_block_wide
   &gen_table &start_table &title_row &add_header_row &add_row &add_col &add_col_link &end_table
...
###################################################################################################################################
   ...
   $sec = &formatID('', 2, $sec);
   my $dmscgi = new CGI;
   my $command = (defined($dmscgi->param("command"))) ? $dmscgi->param("command") : "No command";
   my $documentid = (defined($dmscgi->param("id"))) ? $dmscgi->param("id") - 0 : "No Doc ID";
   my $commentid = (defined($dmscgi->param("commentid"))) ? $dmscgi->param("commentid") - 0 : "No Comment ID";
   $ENV{SCRIPT_NAME} =~ m%.*/(.*)%;
   print STDERR "***** $ENV{'DMSType'} $1 $mon/$mday/$year $hour:$min:$sec $args{userName}/$args{userID}/$args{schema} $ENV{REMOTE_ADDR} $command/$documentid/$commentid\n";

   $args{schema} = uc($args{schema}) unless ($args{schema} eq "None");
   #$args{title} = lc($args{title}) unless ($args{title} eq "None");
   #$args{title} =~ s/ /_/g;
   ##$args{title} =~ s?/?_?g;
   ##my $output = "<script language=javascript><!--\nparent.frames[1].writeTitleBar('$DMSJavaScriptPath', '$DMSImagePath', '$args{title}', '$args{userName}', '$args{userID}', '$args{schema}');\n//-->\n</script>\n";
   ...
   if ($DMSDebug) {
      ...
      my @list = $dmscgi->param();
      foreach my $item (@list) {
          my $val = $dmscgi->param($item);
...
###################################################################################################################################
   ...
   if ($args{remark_type} eq 'document') {
      ...
      $noRemarks .= "Document " . &formatID($DMSType, 6, $args{document});
      ...
      $noRemarks .= "Comment " . &formatID($DMSType, 6, $args{document}) . " / " . &formatID("", 4, $args{comment});
...
###################################################################################################################################
   ...
   if ($args{remark_type} eq 'comment') {
      $args{remark_type} = 'Document';
      $args{formattedid} = &formatID($DMSType, 6, $args{document});
   ...
   if ($args{show_text_box}) {
      my $dmscgi = new CGI;
      my $text = ($args{useForm} && defined($dmscgi->param($args{text_box_name}))) ? $dmscgi->param($args{text_box_name}) : "";
      $out .= "<tr><td height=15> </td></tr>\n";
      $out .= "<tr><td>\n";
      $ENV{SCRIPT_NAME} =~ m%(.*/)(.*)\..*$%;
      my $form = $2;
      my $expand = "<a href=\"javascript:expandTextBox(document.$form.$args{text_box_name},document.$args{text_box_name}_button,'force',5);\">\n";
      $expand .= "<img name=$args{text_box_name}_button border=0 src=$DMSImagePath/expand_button.gif></a>\n";
...
###################################################################################################################################
   ...
   $min = &formatID('', 2, $min);
   $sec = &formatID('', 2, $sec);
   print STDERR "\n $ENV{'DMSType'} DMS error: $mon/$mday/$year $hour:$min:$sec - $username/$userid/$schema - $appError failed:\n$oracleError\n";
