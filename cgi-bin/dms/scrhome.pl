...
#
# $Source: /data/dev/rcs/dms/perl/RCS/scrhome.pl,v $
# $Revision: 1.3 $
# $Date: 2002/06/30 20:34:10 $
# $Author: mccartym $
# $Locker:  $
# $Log: scrhome.pl,v $
# Revision 1.3  2002/06/30 20:34:10  mccartym
# change product id
#
# Revision 1.2  2002/03/15 17:48:44  atchleyb
# updated to handle the fact that the sccb user id was moved into the users table
#
# Revision 1.1  2002/03/08 21:11:08  atchleyb
# Initial revision
#
#
#
...
my $form = $2;
my $urllocation;
my $command = (defined ($dmscgi -> param("command"))) ? $dmscgi -> param("command") : "write_request";
my $username = (defined($dmscgi->param("username"))) ? $dmscgi->param("username") : "";
my $password = (defined($dmscgi->param("password"))) ? $dmscgi->param("password") : "";
my $userid = (defined($dmscgi->param("userid"))) ? $dmscgi->param("userid") : "";
my $error = "";
my $dbh = db_connect();
my ($sccbid) = $dbh -> selectrow_array ("select sccbid from $schema.users where id=$userid");
...
##################################
    ...
    $expand .= "<img name=request_button border=0 src=$DMSImagePath/expand_button.gif></a>\n";
    ...
    $expand .= "<img name=rationale_button border=0 src=$DMSImagePath/expand_button.gif></a>\n";
    print "<tr><td><table border=0 cellpadding=0 cellspacing=0><tr>";
...
###################################
    no strict 'refs';
    my $request = $dmscgi -> param ("request");
    my $priority = $dmscgi -> param ("priority");
    my $rationale = $dmscgi -> param ("rationale");
    my $product = 9;
    my $status = 1;
    my $newid;
    my $activity;
...
#################
    ...
    $sqlstring = "SELECT $schema1.SCRREQUEST_DMS_ID_SEQ.NEXTVAL FROM DUAL";
