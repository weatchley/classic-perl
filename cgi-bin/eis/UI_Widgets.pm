...
#
# $Source: /usr/local/homes/atchleyb/rcs/crd/perl/RCS/UI_Widgets.pm,v $
#
# $Revision: 1.37 $
#
# $Date: 2006/10/26 21:42:49 $
#
# $Author: atchleyb $
#
# $Locker:  $
#
# $Log: UI_Widgets.pm,v $
# Revision 1.37  2006/10/26 21:42:49  atchleyb
# CREQ00047 - The build_date_selection fuction was modified to allow years past 2005
#
# Revision 1.36  2001/10/05 21:16:40  atchleyb
# Added function to generate a report for one provided SCR
# This with code changes in reports.pl fullfills SCREQ 11
#
# Revision 1.35  2001/08/23 00:52:05  mccartym
# slight change in remarks display formatting
#
# Revision 1.34  2001/08/19 00:30:08  mccartym
# remove extra linefeed on remarks display
#
# Revision 1.33  2001/08/18 01:35:26  mccartym
# remove build_text_box function
#
# Revision 1.32  2001/05/24 20:51:34  atchleyb
# updated to not reformat title text
#
# Revision 1.31  2001/05/01 16:41:38  atchleyb
# updated function for text labels to allow display of '/'
#
# Revision 1.30  2001/04/27 21:28:07  atchleyb
# added the textbox resize function to doRemarks
#
# Revision 1.29  2001/04/04 16:55:50  atchleyb
# added two new parameters to the build_select_from_piclist function
#
# Revision 1.28  2001/03/09 17:21:51  atchleyb
# modified to use the text_labels.pl for page titles
#
# Revision 1.27  2000/07/17 18:57:28  atchleyb
# added option for response version on function for printCommentResponse
#
# Revision 1.26  2000/07/14 17:58:31  atchleyb
# changed form method to post for printcommentresponseform
#
# Revision 1.25  2000/07/14 17:49:43  atchleyb
# added function to generate form and javascript required to support a print button for comment/response pairs
#
# Revision 1.24  2000/05/17 23:27:03  mccartym
# add debug info to writeTitleBar
#
# Revision 1.23  2000/04/08 00:11:44  mccartym
# modify error log description format for complete error message
#
# Revision 1.22  2000/04/07 17:04:20  mccartym
# add web server error log diagnostics
#
# Revision 1.21  2000/04/07 15:31:20  mccartym
# write complete error message to activity_log
#
# Revision 1.20  2000/02/22 17:35:50  mccartym
# filter '\n' in remarks to '<br><br>' for display
#
# Revision 1.19  2000/02/10 01:19:49  mccartym
# change text for no remarks found for increased clarity
#
# Revision 1.18  2000/01/14 23:42:06  atchleyb
# replaced all references to EIS with $crdtype
#
# Revision 1.17  1999/11/29 17:07:26  atchleyb
# removed the sources of several "Use of uninitialized value at..." messages
#
# Revision 1.16  1999/11/25 01:52:04  mccartym
# change table header colors
#
# Revision 1.15  1999/11/04 16:04:02  mccartym
# change writeTitleBar() to make javascript call instead of form submit
#
# Revision 1.14  1999/10/27 23:59:49  mccartym
# clean up, remove dead code
# replace headerBar() with writeTitleBar()
#
# Revision 1.13  1999/08/06 22:27:31  mccartym
# changes to errorMessage() and doRemarks()
#
# Revision 1.12  1999/08/02 02:56:54  mccartym
# added formatID(), doRemarks(), and errorMessage() functions
#
# Revision 1.11  1999/07/30 20:25:30  atchleyb
# fixed extra newline in build_textbox
#
# Revision 1.10  1999/07/28 23:07:15  atchleyb
# fixed element name bug in build text box
#
# Revision 1.9  1999/07/26 18:19:15  atchleyb
# Minor comments change
#
# Revision 1.8  1999/07/22 17:52:28  mccartym
# add headerBar function
#
...
sub build_date_selection {
    ...
    my $dayEnd=0; my $monthEnd=0; my $yearEnd=0;
    ($dayEnd, $monthEnd, $yearEnd) = (localtime)[3,4,5];
    ...
    $yearEnd = (($year > $yearEnd) ? $year : $yearEnd);
    ...
    $outstring .= "<select name=$elementname\_year size=1><option value=\"\">\n";
    for (my $i=1999; $i<=($yearEnd+2); $i++) {
...
###################################################################################################################################
   ...
   $sec = &formatID('', 2, $sec);
   my $crdcgi = new CGI;
   my $command = (defined($crdcgi->param("command"))) ? $crdcgi->param("command") : "No command";
   my $documentid = (defined($crdcgi->param("id"))) ? $crdcgi->param("id") - 0 : "No Doc ID";
   my $commentid = (defined($crdcgi->param("commentid"))) ? $crdcgi->param("commentid") - 0 : "No Comment ID";
   $ENV{SCRIPT_NAME} =~ m%.*/(.*)%;
   print STDERR "***** $ENV{'CRDType'} $1 $mon/$mday/$year $hour:$min:$sec $args{userName}/$args{userID}/$args{schema} $ENV{REMOTE_ADDR} $command/$documentid/$commentid\n";

   $args{schema} = uc($args{schema}) unless ($args{schema} eq "None");
   #$args{title} = lc($args{title}) unless ($args{title} eq "None");
   #$args{title} =~ s/ /_/g;
   ##$args{title} =~ s?/?_?g;
   ##my $output = "<script language=javascript><!--\nparent.frames[1].writeTitleBar('$CRDJavaScriptPath', '$CRDImagePath', '$args{title}', '$args{userName}', '$args{userID}', '$args{schema}');\n//-->\n</script>\n";
   ...
   if ($CRDDebug) {
      ...
      my @list = $crdcgi->param();
      foreach my $item (@list) {
          my $val = $crdcgi->param($item);
...
###################################################################################################################################
   ...
   if ($args{remark_type} eq 'document') {
      ...
      $noRemarks .= "Document " . &formatID($CRDType, 6, $args{document});
      ...
      $noRemarks .= "Comment " . &formatID($CRDType, 6, $args{document}) . " / " . &formatID("", 4, $args{comment});
...
###################################################################################################################################
   ...
   if ($args{remark_type} eq 'comment') {
      $args{remark_type} = 'Document';
      $args{formattedid} = &formatID($CRDType, 6, $args{document});
   ...
   if ($args{show_text_box}) {
      my $crdcgi = new CGI;
      my $text = ($args{useForm} && defined($crdcgi->param($args{text_box_name}))) ? $crdcgi->param($args{text_box_name}) : "";
      $out .= "<tr><td height=15> </td></tr>\n";
      $out .= "<tr><td>\n";
      $ENV{SCRIPT_NAME} =~ m%(.*/)(.*)\..*$%;
      my $form = $2;
      my $expand = "<a href=\"javascript:expandTextBox(document.$form.$args{text_box_name},document.$args{text_box_name}_button,'force',5);\">\n";
      $expand .= "<img name=$args{text_box_name}_button border=0 src=$CRDImagePath/expand_button.gif></a>\n";
...
###################################################################################################################################
   ...
   $min = &formatID('', 2, $min);
   $sec = &formatID('', 2, $sec);
   print STDERR "\n $ENV{'CRDType'} CRD error: $mon/$mday/$year $hour:$min:$sec - $username/$userid/$schema - $appError failed:\n$oracleError\n";
