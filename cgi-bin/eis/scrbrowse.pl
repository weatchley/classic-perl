...
#
# $Source: /data/dev/rcs/crd/perl/RCS/scrbrowse.pl,v $
# $Revision: 1.16 $
# $Date: 2003/03/11 16:43:09 $
# $Author: atchleyb $
# $Locker:  $
# $Log: scrbrowse.pl,v $
# Revision 1.16  2003/03/11 16:43:09  atchleyb
# updated to use new pcl schema instead of scm schema
#
# Revision 1.15  2002/11/09 00:12:52  atchleyb
# fixed error in brwose detail
#
# Revision 1.14  2002/11/08 23:58:07  atchleyb
# fixed bug in table name
#
# Revision 1.13  2002/10/22 22:03:57  atchleyb
# updated to mach changes made in scrs for the new scr system
#
# Revision 1.12  2001/09/21 16:46:20  naydenoa
# Modified to accommodate new status
#
# Revision 1.11  2001/09/10 20:16:29  naydenoa
# Corrected product schema - bug is now fixed
#
# Revision 1.10  2001/09/10 15:20:36  naydenoa
# Fixed browse details bug
#
# Revision 1.9  2001/09/04 23:01:04  naydenoa
# checkpoint
#
# Revision 1.8  2001/08/30 21:38:20  naydenoa
# Filtered out CMS requests.
#
# Revision 1.7  2001/07/26 22:35:50  naydenoa
# Changed SCR schema to SCM
#
# Revision 1.6  2001/06/22 15:45:37  naydenoa
# Fixed JavaScript bug in browseDetails
#
# Revision 1.5  2001/06/21 22:03:13  naydenoa
# Added button for report generation.
#
# Revision 1.4  2001/06/18 22:54:48  naydenoa
# Fixed bug in info retrieval on browse details
#
# Revision 1.3  2001/06/18 17:45:11  naydenoa
# Added product to results and details screens
#
# Revision 1.2  2001/06/15 23:19:42  naydenoa
# Changed table header color, took out product
#
# Revision 1.1  2001/06/14 22:18:14  naydenoa
# Initial revision
#
#
...
#################
    ...
    print "<TR><TD COLSPAN=7 BGCOLOR=#cdecff><FONT COLOR=#000099 SIZE=4><B>All CRD Software Change Requests ($total total)</TD></TR>\n";#cdecff
    ...
    my $pick = "select r.id, s.description, s.id, r.submittedby, u.firstname || ' ' || u.lastname, r.description, to_char(r.datesubmitted, 'MM/DD/YYYY'), p.description, pr.name from $schema1.scrrequest r, $schema1.users u, $schema1.scrstatus s, $schema1.scrpriority p, $schema1.product pr where pr.id=1 and r.status=s.id and r.submittedby=u.id and r.priority=p.id and r.product=pr.id order by r.id";
...
#################
    ...
    $sql = "select to_char(r.datesubmitted, 'MM/DD/YYYY'), u.firstname || ' ' ||u.lastname, p.description, s.id, s.description, r.description, r.rationale, pr.name from $schema1.scrrequest r, $schema1.users u, $schema1.scrpriority p, $schema1.scrstatus s, $schema1.product pr where r.id = $rid and u.id=r.submittedby and p.id=r.priority and s.id=r.status and r.product=pr.id";
    $sth = $dbh->prepare($sql);
    $sth->execute();
