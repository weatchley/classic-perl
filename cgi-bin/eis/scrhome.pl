...
#
# $Source: /data/dev/rcs/crd/perl/RCS/scrhome.pl,v $
# $Revision: 1.7 $
...
#
# Revision 1.6  2001/09/04 23:01:21  naydenoa
# checkpoint
#
# Revision 1.5  2001/08/18 00:59:03  mccartym
# add text box expanders
# ,
#
# Revision 1.4  2001/07/26 22:36:29  naydenoa
# Changed SCR schema to SCM from EIS
#
# Revision 1.3  2001/06/18 17:44:52  naydenoa
# Fixed javascript for error handling
#
# Revision 1.2  2001/06/15 23:20:10  naydenoa
# Changed userid to sccbid on insert, took out reference to product SR
#
# Revision 1.1  2001/06/14 22:20:07  naydenoa
# Initial revision
#
#
...
my $form = $2;
my $urllocation;
my $command = (defined ($crdcgi -> param("command"))) ? $crdcgi -> param("command") : "write_request";
my $username = (defined($crdcgi->param("username"))) ? $crdcgi->param("username") : "";
my $password = (defined($crdcgi->param("password"))) ? $crdcgi->param("password") : "";
my $userid = (defined($crdcgi->param("userid"))) ? $crdcgi->param("userid") : "";
my $error = "";
my $dbh = db_connect();
...
##################################
    ...
    $expand .= "<img name=request_button border=0 src=$CRDImagePath/expand_button.gif></a>\n";
    ...
    $expand .= "<img name=rationale_button border=0 src=$CRDImagePath/expand_button.gif></a>\n";
    print "<tr><td><table border=0 cellpadding=0 cellspacing=0><tr>";
...
###################################
    no strict 'refs';
    my $request = $crdcgi -> param ("request");
    my $priority = $crdcgi -> param ("priority");
    my $rationale = $crdcgi -> param ("rationale");
    my $product = 1;
    my $status = 1;
    my $newid;
    my $activity;
