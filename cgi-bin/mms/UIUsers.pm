# UI User functions
#
# $Source: /usr/local/www/gov.ymp.intradev/rcs/mms/perl/RCS/UIUsers.pm,v $
#
# $Revision: 1.5 $
#
...
#
# Revision 1.3  2006/05/17 23:26:03  atchleyb
# CR0026 - added disabled flag to display for inactive users
#
# Revision 1.2  2004/05/07 16:00:15  atchleyb
# Updated with new password security changes
#
# Revision 1.1  2003/11/12 20:35:20  atchleyb
# Initial revision
#
#
#
...
###################################################################################################################################
   ...
   for (my $i=0; $i < $#users; $i++) {
      my ($id, $username, $lastname, $firstname, $organization, $isActive) = ($users[$i]{id},$users[$i]{username},$users[$i]{lastname},$users[$i]{firstname},$users[$i]{organization},$users[$i]{isactive});
      my $fullName = &get_fullname($args{dbh}, $args{schema}, $id);
      my $prompt = "Click here to display information for $fullName";
      $output .= &startRow . &addCol(value => $fullName . (($isActive eq 'T') ? "" : "<br>(Disabled)"), url => "javascript:displayUser($id)", prompt => $prompt);
...
###################################################################################################################################
    ...
    my %valueHash = (
       ...
       onlyactive => (defined($mycgi->param("onlyactive"))) ? $mycgi->param("onlyactive") : "F",
#       privlist => $mycgi->param("privlist"),
    ));
    my @privList = $mycgi->param("privlist");
    $valueHash{privlist} = \@privList;
    my @deptList = $mycgi->param("deptlist");
    $valueHash{deptlist} = \@deptList;
...
###################################################################################################################################
    ...
    $uservalues[9][0] ='<b>Departments:</b>';
    $userInfo{departments} =~ s/\t/<br>/g;
    $uservalues[9][1] ="<b>" . $userInfo{departments};
    $uservalues[10][0] ='<b>Roles:</b>';
    $userInfo{roles} =~ s/\t/<br>/g;
    $uservalues[10][1] ="<b>" . $userInfo{roles};
    $uservalues[11][0] = '<b>Status:</b>';
    $uservalues[11][1] = '<b>' . (($userInfo{isactive} eq 'T') ? 'Active' : 'Inactive') . '</b>';
    $uservalues[12][0] = '<b>Failed Logins:</b>';
    $uservalues[12][1] = "<b>" . $userInfo{failedattempts} . "</b>";
    $uservalues[13][0] = '<b>Lockout Time:</b>';
    $uservalues[13][1] = '<b>' . ((defined($userInfo{lockout})) ? $userInfo{lockout} : "&nbsp;") . '</b>';
...
END_OF_BLOCK
    $output .= "<tr><td><b>Active Only:</b><td><input type=checkbox name=onlyactive value='T'" . (($args{onlyActive} eq 'T') ? " checked" : "") . "></td></tr>\n";
    if ($args{selectedUser} != 0) {
...
###################################################################################################################################
    ...
    if ($args{type} eq 'update') {
        ...
        $output .= &doUpdateUserSelect(dbh => $args{dbh}, schema => $args{schema}, title => $args{title}, form => $args{form}, 
                                   userID => $args{userID}, settings => \%settings, selectedUser => $id, onlyActive => $settings{onlyactive});
    }

    $output .= "<input type=hidden name=approvalcount value=$userInfo{approvalCount}>\n";
    $output .= "<input type=hidden name=rolecount value=$userInfo{roleCount}>\n";
...
# departments
    ...
    tie my %departments, "Tie::IxHash";
    %departments = %{&getDeptHash (dbh => $args{dbh}, schema => $args{schema})};
    tie my %currdepts, "Tie::IxHash";
    my @depts = split("\t",$userInfo{departmentids} . '0');
    for (my $i=0; $i<$#depts; $i++) {
        if ($depts[$i] > 0) {
            $currdepts{$depts[$i]} = $userInfo{"dept$depts[$i]"};
        }
    }
    $output .= "<tr><td colspan=4 align=center>\n";
    $output .= build_dual_select ('deptlist', "$args{form}", \%departments, \%currdepts, "<b>Available Departments</b>", "<b>Selected Departments</b>", 0);
    $output .= "</td></tr>\n";
...
# Control buttons
    ...
    if ($args{type} eq 'update') {
        if ($userInfo{isactive} eq 'T') {
            my $hasRolesApprovals = (($userInfo{approvalCount} > 0 || $userInfo{roleCount} > 0 || $userInfo{managerCount} > 0) ? "disabled alt='Test'" : "");
            my $disableMessage = (($userInfo{approvalCount} > 0 || $userInfo{roleCount} > 0 || $userInfo{managerCount} > 0) ? "<a href=# title='User must have all roles removed, approvals reassigned, and not be a team lead'><b>?</b></a>" : "");
            #$output .= "<input type=submit name=disable value=\"Disable Account\" onClick=\"document.$args{form}.command.value='disableuser'\"> &nbsp;\n";
            $output .= "<input type=button name=disable value=\"Disable Account\" onClick=\"disableSubmit(document.$args{form})\" $hasRolesApprovals>$disableMessage &nbsp;\n";
        } else {
...
// javascript form verification routine
    ...
    for (index=0; index < f.deptlist.length-1;index++) {
        f.deptlist.options[index].selected = true;
    }
    if (msg != "") {
      alert (msg);
    } else {
...
// function to see if user can be disabled
    var msg = "";
    if (f.approvalcount.value > 0) {
      msg += "User has pending approvals.\\n";
    }
    if (f.rolecount.value > 0) {
      msg += "User has roles assigned.\\n";
    }
    if (msg != "") {
      alert (msg);
    } else {
      f.command.value='disableuser';
      f.submit();
    }
}
//--></script>
...
###################################################################################################################################
    ...
    $output .= &doUpdateUserSelect(dbh => $args{dbh}, schema => $args{schema}, title => $args{title}, form => $args{form}, excludeDevelopers => 'F',
