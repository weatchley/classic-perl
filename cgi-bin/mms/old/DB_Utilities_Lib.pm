...
use vars qw($DBUser $MMSConnectPath $MMSServer);
...
$DBUser = $ENV{'DBUser'};
$MMSConnectPath = $ENV{'MMSConnectPath'};
$MMSServer = $ENV{'MMSServer'};
#$schema = $ENV{'SCHEMA'};
...
sub getOracleID {
    ...
    if (open (FH, "$MMSConnectPath |")) {
        ...
        chomp $password;
        $password = decrypt(text => $password, key => $key);
        close (FH);
    } else {
        $username = "null";
        $password = "null";
