# Library of UI widget routines for cgi scripts (used first for MMS)
...
###################################################################################################################################
   ...
   $sec = &formatID('', 2, $sec);
   my $mmscgi = new CGI;
   my $command = (defined($mmscgi->param("command"))) ? $mmscgi->param("command") : "No command";
   my $documentid = (defined($mmscgi->param("id"))) ? $mmscgi->param("id") - 0 : "No Doc ID";
   my $commentid = (defined($mmscgi->param("commentid"))) ? $mmscgi->param("commentid") - 0 : "No Comment ID";
   $ENV{SCRIPT_NAME} =~ m%.*/(.*)%;
   print STDERR "***** $ENV{'MMSType'} $1 $mon/$mday/$year $hour:$min:$sec $args{userName}/$args{userID}/$args{schema} $ENV{REMOTE_ADDR} $command/$documentid/$commentid\n";

   $args{schema} = uc($args{schema}) unless ($args{schema} eq "None");
   #$args{title} = lc($args{title}) unless ($args{title} eq "None");
   #$args{title} =~ s/ /_/g;
   ##$args{title} =~ s?/?_?g;
   ##my $output = "<script language=javascript><!--\nparent.frames[1].writeTitleBar('$MMSJavaScriptPath', '$MMSImagePath', '$args{title}', '$args{userName}', '$args{userID}', '$args{schema}');\n//-->\n</script>\n";
   ...
   if ($MMSDebug) {
      ...
      my @list = $mmscgi->param();
      foreach my $item (@list) {
          my $val = $mmscgi->param($item);
...
###################################################################################################################################
   ...
   if ($args{remark_type} eq 'document') {
      ...
      $noRemarks .= "Document " . &formatID($MMSType, 6, $args{document});
      ...
      $noRemarks .= "Comment " . &formatID($MMSType, 6, $args{document}) . " / " . &formatID("", 4, $args{comment});
...
###################################################################################################################################
   ...
   if ($args{remark_type} eq 'comment') {
      $args{remark_type} = 'Document';
      $args{formattedid} = &formatID($MMSType, 6, $args{document});
   ...
   if ($args{show_text_box}) {
      my $mmscgi = new CGI;
      my $text = ($args{useForm} && defined($mmscgi->param($args{text_box_name}))) ? $mmscgi->param($args{text_box_name}) : "";
      $out .= "<tr><td height=15> </td></tr>\n";
      $out .= "<tr><td>\n";
      $ENV{SCRIPT_NAME} =~ m%(.*/)(.*)\..*$%;
      my $form = $2;
      my $expand = "<a href=\"javascript:expandTextBox(document.$form.$args{text_box_name},document.$args{text_box_name}_button,'force',5);\">\n";
      $expand .= "<img name=$args{text_box_name}_button border=0 src=$MMSImagePath/expand_button.gif></a>\n";
...
###################################################################################################################################
   ...
   $min = &formatID('', 2, $min);
   $sec = &formatID('', 2, $sec);
   print STDERR "\n $ENV{'MMSType'} MMS error: $mon/$mday/$year $hour:$min:$sec - $username/$userid/$schema - $appError failed:\n$oracleError\n";
