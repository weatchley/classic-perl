...
#################
    ...
    print "<TR><TD COLSPAN=7 BGCOLOR=#cdecff><FONT COLOR=#000099 SIZE=4><B>All BMS Software Change Requests ($total total)</TD></TR>\n";#cdecff
    ...
    my $pick = "select r.id, s.description, s.id, r.submittedby, u.firstname || ' ' || u.lastname, r.description, to_char(r.datesubmitted, 'MM/DD/YYYY'), p.description, pr.name from $schema1.scrrequest r, $schema1.scruser u, $schema1.scrstatus s, $schema1.scrpriority p, $schema1.scrproduct pr where pr.id=1 and r.status=s.id and r.submittedby=u.id and r.priority=p.id and r.product=pr.id order by r.id";
...
#################
    ...
    $sql = "select to_char(r.datesubmitted, 'MM/DD/YYYY'), u.firstname || ' ' ||u.lastname, p.description, s.id, s.description, r.description, r.rationale, pr.name from $schema1.scrrequest r, $schema1.scruser u, $schema1.scrpriority p, $schema1.scrstatus s, $schema1.scrproduct pr where r.id = $rid and u.id=r.submittedby and p.id=r.priority and s.id=r.status and r.product=pr.id";
    $sth = $dbh->prepare($sql);
    $sth->execute();
