...
my $form = $2;
my $urllocation;
my $command = (defined ($bmscgi -> param("command"))) ? $bmscgi -> param("command") : "write_request";
my $username = (defined($bmscgi->param("username"))) ? $bmscgi->param("username") : "";
my $password = (defined($bmscgi->param("password"))) ? $bmscgi->param("password") : "";
my $userid = (defined($bmscgi->param("userid"))) ? $bmscgi->param("userid") : "";
my $error = "";
my $dbh = db_connect();
...
##################################
    print "<body background=$BMSImagePath/background.gif text=$BMSFontColor link=#0000ff vlink=#0000ff alink=#0000ff topmargin=0 leftmargin=0>\n";
    ...
    $expand .= "<img name=request_button border=0 src=$BMSImagePath/expand_button.gif></a>\n";
    ...
    $expand .= "<img name=rationale_button border=0 src=$BMSImagePath/expand_button.gif></a>\n";
    print "<tr><td><table border=0 cellpadding=0 cellspacing=0><tr>";
...
###################################
    no strict 'refs';
    my $request = $bmscgi -> param ("request");
    my $priority = $bmscgi -> param ("priority");
    my $rationale = $bmscgi -> param ("rationale");
    my $product = 1;
    my $status = 1;
    my $newid;
    my $activity;
