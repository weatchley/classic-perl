...
my $message = '';
my $instructionsColor = $BMSFontColor;
my $title = "Utilities";
...
<head>
   ...
   <script src=$BMSJavaScriptPath/utilities.js></script>
   <script src=$BMSJavaScriptPath/widgets.js></script>
...
print "\n</head>\n";
print "<body background=$BMSImagePath/background.gif text=$BMSFontColor link=#0000ff vlink=#0000ff alink=#0000ff topmargin=0 leftmargin=0><center>\n";
...
if ($command eq 'view_errors' || $command eq 'view_activity') {
    ...
    my $logOption = (defined($bmscgi->param("logOption"))) ? $bmscgi->param("logOption") : "today";
    my $logactivity = (defined($bmscgi->param("logactivity"))) ? $bmscgi->param("logactivity") : "all";
    my $selecteduser = (defined($bmscgi->param("selecteduser"))) ? $bmscgi -> param ("selecteduser") : -1;
