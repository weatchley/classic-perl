...
my $form = $2;
my $urllocation;
my $username = (defined($mmscgi->param("username"))) ? uc($mmscgi->param("username")) : "GUEST";
my $password = (defined($mmscgi->param("password"))) ? $mmscgi->param("password") : "GUEST";
my $userid = 0;
my $sessionID;
my $error = "";
my $dbh;
...
<head>
   ...
   <title>RSIS Materials Management System</title>
...
# make cgiresults size 0 on prod
   print "<frameset rows=115,60,*," . (($MMSDebug) ? "5" : "1") ." border=0 framespacing=0>\n";
   ...
   # my $message = "The $MMSType Comment/Response Database will be unavailable between 5:00 and 7:00 PM on Friday, July 21 for maintenance";
   my $message = "";
   my $onload = (($command eq 'makeform') && $message) ? "onload=\"alert('$message')\"" : "" ;
   print "<body background=$MMSImagePath/background.gif text=$MMSFontColor link=#0000ff vlink=#0000ff alink=#0000ff topmargin=0 leftmargin=0 $onload>\n";
   print "<font face=$MMSFontFace color=$MMSFontColor><center>\n";
   ...
   if ($command ne "login_action") {
      ...
      eval {
         ...
         if ($status != 1) {
            ...
            $sessionID = &sessionCreate(dbh => $dbh, schema => $schema, userID => $userid, application => $MMSType, timeout => $MMSTimeout);
#print "\n<!-- $sessionID -->\n\n";
