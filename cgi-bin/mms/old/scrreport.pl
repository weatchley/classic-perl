...
#################
    ...
    my $pick = "select r.id, s.id, s.description, r.submittedby, u.firstname || ' ' || u.lastname, r.description, to_char(r.datesubmitted, 'MM/DD/YYYY'), p.description, r.rationale, pr.name from $schema1.scrrequest r, $schema1.user u, $schema1.scrstatus s, $schema1.scrpriority p, $schema1.product pr where pr.id=1 and r.status=s.id and r.submittedby=u.id and r.priority=p.id and pr.id=r.product order by r.id";
