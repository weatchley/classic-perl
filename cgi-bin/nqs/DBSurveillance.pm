#
# $Source: /usr/local/homes/higashis/www/gov.doe.ocrwm.ydappdev/rcs/qa/perl/RCS/DBSurveillance.pm,v $
#
# $Revision: 1.16 $ 
#
...
# Revision 1.16  2009/09/22 18:20:47  higashis
# major version release.
#
# Revision 1.15  2009/03/24 20:46:45  patelr
# added delete attachment function
#
# Revision 1.14  2009/02/10 20:54:02  patelr
# edited doProcessCreateSurveillance and doProcessUpdateSurveillance to allow editing of reports issued by lead lab
#
# Revision 1.13  2009/02/04 00:14:13  patelr
# Added filename validation for ll and snl prefixes
#
# Revision 1.12  2009/01/16 21:06:25  patelr
# revised doProcessCreateSurveillance to include attachment code for file upload
#
...
###################################################################################################################################
    ...
    my $issuedbystr = $args{selection} eq 'BSC' ? "and issuedby_org_id = 1 " : $args{selection} eq 'SNL' ? "and issuedby_org_id = 33 " : $args{selection} eq 'OQA' ? "and issuedby_org_id = 28 ": $args{selection} eq 'MQA' ? "and issuedby_org_id = 17 " : "";
    my @surveillanceList;
    ...
    #$sqlcode .= "ORDER BY issuedby_org_id, id ";
   	 ...
   	 my $sqlcode = "";
   	 $sqlcode = "SELECT id,fiscal_year,initial_contact,elements,team_members,team_lead_id,issuedto_org_id,issuedby_org_id,";
    $sqlcode .= "scope,to_char(approval_date,'MM/DD/YYYY'),cancelled,to_char(forecast_date,'MM/YYYY'),to_char(begin_date,'MM/DD/YYYY'),";
    $sqlcode .= "to_char(end_date,'MM/DD/YYYY'),to_char(completion_date,'MM/DD/YYYY'),surveillance_seq,int_ext,status,";
    $sqlcode .= "qard_elements, reschedule, overall_results,overall,to_char(estbegin_date,'MM/YYYY'),";
    $sqlcode .= "to_char(estend_date,'MM/DD/YYYY'),procedures,state,adequacy,implementation,effectiveness,title,reportlink,dblink,mol ";
    ...
    $sqlcode .=" fiscal_year = substr($args{fiscalyear},-2,2) $issuedbystr $args{where} AND ";
    }else{
    $sqlcode .=" fiscal_year = substr($args{fiscalyear},-1,2) $issuedbystr $args{where} AND ";
    }
    
    $sqlcode .="issuedby_org_id = 28 ";    
    $sqlcode .="UNION ALL ";
    $sqlcode .= "SELECT id,fiscal_year,initial_contact,elements,team_members,team_lead_id,issuedto_org_id,issuedby_org_id,";
    $sqlcode .= "scope,to_char(approval_date,'MM/DD/YYYY'),cancelled,to_char(forecast_date,'MM/YYYY'),to_char(begin_date,'MM/DD/YYYY'),";
    $sqlcode .= "to_char(end_date,'MM/DD/YYYY'),to_char(completion_date,'MM/DD/YYYY'),surveillance_seq,int_ext,status,";
...
###################################################################################################################################
    ...
    #$sqlstring .= "WHERE l.id=s.location_id AND s.fiscal_year = substr($args{fiscalyear},-1,2) AND s.surveillance_id = $args{survID} ";
            ...
            if($args{fiscalyear}>2009){
			 $sqlstring .= "WHERE l.id=s.location_id AND s.fiscal_year = substr($args{fiscalyear},-2,2) AND s.surveillance_id = $args{survID} ";
			 }else{
			 $sqlstring .= "WHERE l.id=s.location_id AND s.fiscal_year = substr($args{fiscalyear},-1,2) AND s.surveillance_id = $args{survID} ";
			 }
...
###################################################################################################################################
    ...
    #$sqlstring .= "WHERE o.id=s.organization_id AND s.fiscal_year = substr($args{fiscalyear},-1,2) AND s.surveillance_id = $args{survID} ";
             ...
             if($args{fiscalyear}>2009){
			 $sqlstring .= "WHERE o.id=s.organization_id AND s.fiscal_year = substr($args{fiscalyear},-2,2) AND s.surveillance_id = $args{survID} ";
			 }else{
			 $sqlstring .= "WHERE o.id=s.organization_id AND s.fiscal_year = substr($args{fiscalyear},-1,2) AND s.surveillance_id = $args{survID} ";
			 }
    
    #$sqlstring .= "WHERE o.abbr in ('OCRWM','M&O','Lead Lab','OQA') ";
#print "<br>\n $sqlstring \n";
    my $csr = $args{dbh}->prepare($sqlstring);
    my $status = $csr->execute;
...
###################################################################################################################################
    ...
    #$sqlstring .= "WHERE o.id=s.suborganization_id AND s.fiscal_year = substr($args{fiscalyear},-1,2) AND s.surveillance_id = $args{survID} ";
                 ...
                 if($args{fiscalyear}>2009){
			 $sqlstring .= "WHERE o.id=s.suborganization_id AND s.fiscal_year = substr($args{fiscalyear},-2,2) AND s.surveillance_id = $args{survID} ";
			 }else{
			 $sqlstring .= "WHERE o.id=s.suborganization_id AND s.fiscal_year = substr($args{fiscalyear},-1,2) AND s.surveillance_id = $args{survID} ";
			 }
...
###################################################################################################################################
    ...
    $sqlstring .="AND (initcap(city) = 'Las Vegas' OR initcap(city) = 'Washington') ";
    ...
    if ($args{intext} eq 'I') {
    	...
    	$sqlstring .= "AND abbr in ('OCRWM','M&O','Lead Lab','OQA') ";
...
###################################################################################################################################
    ...
    #$sqlstring .= "WHERE q.id=s.supplier_id AND s.fiscal_year = substr($args{fiscalyear},-1,2) AND s.surveillance_id = $args{survID} ";
                     ...
                     if($args{fiscalyear}>2009){
			 $sqlstring .= "WHERE q.id=s.supplier_id AND s.fiscal_year = substr($args{fiscalyear},-2,2) AND s.surveillance_id = $args{survID} ";
			 }else{
			 $sqlstring .= "WHERE q.id=s.supplier_id AND s.fiscal_year = substr($args{fiscalyear},-1,2) AND s.surveillance_id = $args{survID} ";
			 }
...
###################################################################################################################################
    ...
    #$sqlstring .= "WHERE fiscal_year = substr($args{fiscalyear},-1,2) AND surveillance_id = $args{recordID} ";
             ...
             if($args{fiscalyear}>2009){
			 $sqlstring .= "WHERE fiscal_year = substr($args{fiscalyear},-2,2) AND surveillance_id = $args{recordID} ";
			 }else{
			 $sqlstring .= "WHERE fiscal_year = substr($args{fiscalyear},-1,2) AND surveillance_id = $args{recordID} ";
			 }
...
###################################################################################################################################
    ...
    $sqlstring .= "WHERE  (p.privilege = 'Team Lead' and p.id = up.privilege ";
...
###################################################################################################################################
    ...
    print STDERR  "\n**************************************************** newfyselect -- $settings{newfyselect} *********************************************************\n";
    
    my $fy = 0;
    my ($survid) = 0;
    if ($settings{newfyselect}>9){
    	 $fy = $settings{newfyselect};
    	$survid = $args{dbh}->selectrow_array("select $args{schema}.surveillance_" . $fy . "_seq.nextval from dual");
    }else{
    	 $fy =substr($settings{newfyselect},-1,2);
    	 $survid = $args{dbh}->selectrow_array("select $args{schema}.surveillance_" . lpadzero($fy,2) . "_seq.nextval from dual");
    }
...
#print "\n-->select $args{schema}.surveillance_" . lpadzero($fy,2) . "_seq.nextval from dual<br>\n";
    ...
    if($newFilename){
      ...
      my $testname1 = "oq $yearf s";
      $testname1 =~ s/ //g; 
      my $testname2 = "ll $yearf s";
      $testname2 =~ s/ //g; 
      my $testname3 = "snl $yearf s";
      $testname3 =~ s/ //g; 
      my $inputname = substr($newFilename,0,-7);
      if(substr($inputname,-8) ne $testname1 && substr($inputname,-8) ne $testname2 && substr($inputname,-8) ne $testname3){
         print "<script language = 'JavaScript'>alert(\"File \'$newFilename\' does not meet filenaming conventions!\\nNaming convention is [department name] PLUS [last two digits of year] PLUS ['s' for surveillance document] PLUS [three digit document number]\\nFilename should begin with [\'oq\' or \'ll\' or \'snl\'][last two digits of year][\'s\']\\nFor example, \'oq09s001.pdf\'\");</script>\n";
         return;
      }
   ...
   # $sqlcode .= defined($notes) ? ",$notes " : "";
    ...
    for (my $i = 0; $i < $insertrecords; $i++) {
    	...
    	#$sqlcode .= ")\nVALUES ( $i + 1,$fy,-1,2),$survid";
    	
    	my $fy3 = $args{fiscalyear};
    	
    	if($fy3>9){
    	$sqlcode .= ")\nVALUES ( $i + 1,$fy3,$survid";
    	}else{
    	$sqlcode .= ")\nVALUES ( $i + 1,substr($fy3,-1,2),$survid";	
    	}
    ...
    my $acnym = $settings{issuedby} == 28 ? "OQA" : ($settings{issuedby} == 33) ? "LQA" : ($settings{issuedby} == 17) ? "MQA" : "BQA";
    my $displayid;
    my $surtype = ($settings{int_ext} eq "I" ? "SI" : "SE");
    #if (acnym eq "OQA"){
    $displayid = $acnym."-".$surtype."-".substr($args{fiscalyear},-2)."-".lpadzero($survseq,3); 
    	#$displayid = $acnym."-".$surtype."-".substr($args{fiscalyear},-2)."-".$survseq;
    #}elsif(acnym eq "LQA"){
    #$displayid = $acnym."-".$surtype."-".substr($args{fiscalyear},-2)."-".lpadzero($survseq,3); 
    #}elsif(acnym eq "MQA"){
    #$displayid = $acnym."-".$surtype."-".substr($args{fiscalyear},-2)."-".lpadzero($survseq,3); 
    #}elsif(acnym eq "BQA"){
    #$displayid = $acnym."-".$surtype."-".substr($args{fiscalyear},-2)."-".lpadzero($survseq,2); 
    #}
    
    &log_nqs_activity($args{dbh},$args{schema},'F',$args{userID},"Surveillance $displayid ($survid) inserted by $settings{username}");
...
###################################################################################################################################
   ...
   # $sqlcode .= "cancelled = '$settings{cancelled}', ";
    ...
    if($args{fiscalyear}>2009){
    $sqlcode .= "WHERE id = $args{survID} AND fiscal_year = substr($args{fiscalyear},-2,2) ";
    }else{
    $sqlcode .= "WHERE id = $args{survID} AND fiscal_year = substr($args{fiscalyear},-1,2) ";
    }
 ...
 #   print "\n-->$sqlcode\n";
    ...
    #$sqlcode .= "WHERE surveillance_id = $args{survID} AND fiscal_year = substr($args{fiscalyear},-1,2) ";
     ...
     if($args{fiscalyear}>2009){
    $sqlcode .= "WHERE surveillance_id = $args{survID} AND fiscal_year = substr($args{fiscalyear},-2,2) ";
    }else{
    $sqlcode .= "WHERE surveillance_id = $args{survID} AND fiscal_year = substr($args{fiscalyear},-1,2) ";
    }
  ...
  #  print "\n--><br>$sqlcode\n";
    ...
    for (my $i = 0; $i < $insertrecords; $i++) {
    	...
    	#$sqlcode .= ")\nVALUES ( $i + 1,substr($args{fiscalyear},-1,2),$args{survID}";
    	     ...
    	     if($args{fiscalyear}>2009){
		    $sqlcode .= ")\nVALUES ( $i + 1,substr($args{fiscalyear},-2,2),$args{survID}";
		    }else{
		    $sqlcode .= ")\nVALUES ( $i + 1,substr($args{fiscalyear},-1,2),$args{survID}";
		    }
...
#print "\n--><br>$sqlcode\n";
		...
		if ($deficiencystr[$i-1]) {
			my $deficiency = $args{dbh}->quote($deficiencystr[$i-1]);
    			...
    			#$sqlcode .= "VALUES ( $i,$args{survID},substr($args{fiscalyear},-1,2),1,$deficiency)";
    			    	     ...
    			    	     if($args{fiscalyear}>2009){
							    $sqlcode .= "VALUES ( $i,$args{survID},substr($args{fiscalyear},-2,2),1,$deficiency)";
							    }else{
							    $sqlcode .= "VALUES ( $i,$args{survID},substr($args{fiscalyear},-1,2),1,$deficiency)";
							    }
    ...
    &log_nqs_activity($args{dbh},$args{schema},'F',$args{userID},"Surveillance $settings{displayid} ($args{survID}) updated");
...
###################################################################################################################################
    ...
    #$sqlcode2 = "DELETE from $args{schema}.surveillance WHERE id = $args{survID} AND fiscal_year = substr($args{fiscalyear},-1,2)";
								...
								if($args{fiscalyear}>2009){
							    $sqlcode2 = "DELETE from $args{schema}.surveillance WHERE id = $args{survID} AND fiscal_year = substr($args{fiscalyear},-2,2)";
							    }else{
							   $sqlcode2 = "DELETE from $args{schema}.surveillance WHERE id = $args{survID} AND fiscal_year = substr($args{fiscalyear},-1,2)";
							    }
   ...
   $csr->execute;
    ...
    $args{dbh}->commit;
    $csr->finish;
    &log_nqs_activity($args{dbh},$args{schema},'F',$args{userID},"Surveillance $args{displayid} ($args{survID}) deleted for fy $args{fiscalyear}");
...
###################################################################################################################################
sub doDeleteAttachment {  # routine to delete an attachment
###################################################################################################################################
    my %args = (
        survID => 0,  # null
        fiscalyear => 50,
