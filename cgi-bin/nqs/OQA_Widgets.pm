...
#
# $Source: /data/dev/rcs/nqs/perl/RCS/OQA_Widgets.pm,v $
#
# $Revision: 1.2 $
#
# $Date: 2001/12/08 00:08:38 $
#
# $Author: johnsonc $
#
# $Locker:  $
#
# $Log: OQA_Widgets.pm,v $
# Revision 1.2  2001/12/08 00:08:38  johnsonc
# Added build_dual_select_stacked function.
#
...
@EXPORT = qw (&build_dual_select &build_dual_select_stacked &build_select_from_piclist &build_drop_box &build_date_selection &build_text_box &build_radio_block
 &build_checkbox_textbox_lock &build_selectbox_textbox_lock &gen_table &start_table &title_row &add_header_row &add_row &add_record_link
 &add_col &add_col_link &end_table &nbspaces &lpadzero &errorMessage &getDisplayString &breakUpLongWords);
@EXPORT_OK = qw(&build_dual_select &build_dual_select_stacked &build_select_from_piclist &build_drop_box &build_date_selection &build_text_box &build_radio_block
 &build_checkbox_textbox_lock &build_selectbox_textbox_lock &gen_table &start_table &title_row &add_header_row &add_row &add_record_link
 &add_col &add_col_link &end_table &nbspaces &lpadzero &errorMessage &getDisplayString &breakUpLongWords);
%EXPORT_TAGS =(
    Functions => [qw(&build_dual_select &build_dual_select_stacked &build_select_from_piclist &build_drop_box &build_date_selection &build_text_box &build_radio_block
 &build_checkbox_textbox_lock &build_selectbox_textbox_lock &gen_table &start_table &title_row &add_header_row &add_row &add_record_link
 &add_col &add_col_link &end_table &nbspaces &lpadzero &errorMessage &getDisplayString &breakUpLongWords) ]
);
...
# 'add_record_link'
# (output string) = &add_record_link( (url)  );
# 'end_table'
...
# remove entrys from left table that exist in right table
    ...
    $outstring .= "\n<!-- set up $elementname -->\n";
    $outstring .= "<table border=0 align=center><tr><td valign=top>\n";
    $outstring .= "$leftname<br>\n";
    $outstring .= "<!-- the ondblclick event calls the javascript to move selected element to twin option box -->\n";
    $outstring .= "<select multiple name=avail$elementname size=5 ondblclick=\"ViewSelected('$action');\" onClick=\"document.$formname.$elementname.selectedIndex = -1;\">\n";
    foreach $username (sort keys %avail) {
        $outstring .= "<option value=\"$avail{$username}\">" . $username . "\n";
    }
    ...
    if ($action eq "modify_selected") {
    		$outstring .= "<input type=button value=\"-->\" name=select$elementname  onClick=\"ChangeInactive('$action'); process_multiple_dual_select_option(document.$formname.avail$elementname,document.$formname.$elementname,'move');\"><br>\n";
    		$outstring .= "<input type=button value=\"<--\" name=deselect$elementname  onClick=\"ChangeActive('$action'); process_multiple_dual_select_option(document.$formname.$elementname,document.$formname.avail$elementname,'move');\"><br>\n";
    }
...
#    $outstring .= ");\"><br>\n";
    ...
    $outstring .= "<!-- the ondblclick event calls the javascript to move selected element to twin option box -->\n";
    $outstring .= "<select multiple name=$elementname size=5 ondblclick=\"ViewSelected('$action');\" onClick=\"document.$formname.avail$elementname.selectedIndex = -1;\"";
#    for (my $i=0; $i<=$#lockedvals; $i++) {
#        $outstring .= ", '$lockedvals[$i]'";
#    }
    $outstring .= ")\">\n";
    foreach $username (sort keys %selected) {
        $outstring .= "<option value = \"$selected{$username}\">" . $username . "\n";
    }
    
    $outstring .= "<!-- this is used to force the size of the option box -->\n";
    $outstring .= "<option value=\"\">&nbsp &nbsp &nbsp &nbsp &nbsp &nbsp &nbsp &nbsp &nbsp &nbsp &nbsp &nbsp &nbsp &nbsp &nbsp &nbsp &nbsp &nbsp &nbsp &nbsp &nbsp &nbsp &nbsp &nbsp\n";
    $outstring .= "</select>\n";
    $outstring .= "</td></tr></table><br>\n";
    return ($outstring);
}


# routine to build a stacked dual selection box from hashes passed to it
sub build_dual_select_stacked {
    my $elementname=$_[0];
    my $formname = $_[1];
    my $availref = $_[2];
    my $selectedref = $_[3];
...
# remove entrys from left table that exist in right table
    ...
    $outstring .= "$leftname<br>\n";
    $outstring .= "<!-- the ondblclick event calls the javascript to move selected element to twin option box -->\n";
    $outstring .= "<select name=avail$elementname size=5 ondblclick=\"ViewSelected('$action');\" onClick=\"document.$formname.$elementname.selectedIndex = -1;\">\n";
    foreach $username (sort keys %avail) {
        $outstring .= "<option value=\"$avail{$username}\">" . $username . "\n";
    }
    $outstring .= "<!-- this is used to force the size of the option box -->\n";
    $outstring .= "<option value=\"\">&nbsp &nbsp &nbsp &nbsp &nbsp &nbsp &nbsp &nbsp &nbsp &nbsp &nbsp &nbsp &nbsp &nbsp &nbsp &nbsp &nbsp &nbsp &nbsp &nbsp &nbsp &nbsp &nbsp &nbsp\n";
    $outstring .= "</select>\n";
    $outstring .= "</td></tr>"; #<tr><td align=center>\n";
#    $outstring .= "<!-- the select and deselect buttons call the javascript to move selected element to twin option box -->\n";
#    $outstring .= "<input type=button value=\"-->\" name=select$elementname  onClick=\"ChangeInactive(); process_multiple_dual_select_option(document.$formname.avail$elementname,document.$formname.$elementname,'move');\">\n";
#    $outstring .= "<input type=button value=\"<--\" name=deselect$elementname name=deselect$elementname  onClick=\"ChangeActive(); process_multiple_dual_select_option(document.$formname.$elementname,document.$formname.avail$elementname,'move'";
#    for (my $i=0; $i<=$#lockedvals; $i++) {
#        #$outstring .= ", '$lockedvals[$i]'";
#    }
#    $outstring .= ")\"><br>\n";
#    $outstring .= "</td></tr>";
    $outstring .= "<tr><td valign=top>\n";
    $outstring .= "$rightname<br>\n";
    $outstring .= "<!-- the ondblclick event calls the javascript to move selected element to twin option box -->\n";
    $outstring .= "<select name=$elementname size=5 ondblclick=\"ViewSelected('$action');\" onClick=\"document.$formname.avail$elementname.selectedIndex = -1;\")>\n";
#    for (my $i=0; $i<=$#lockedvals; $i++) {
#        $outstring .= ", '$lockedvals[$i]'";
#    }
#    $outstring .= ")\">\n";
...
sub gen_table {
    ...
    for (my $i=1; $i <= $#table_array; $i++) {
        ...
        $outstring .= "<tr bgcolor=$color><td valign=top>$table_array[$i][0]</td><td>$table_array[$i][1]</td></tr>\n";
    }
    $outstring .= "</table>\n";
...
# get column widths if they were passed in
    for (my $i = 2; $i<=($cols+1);$i++) {
        my $j=$i-1;
        if ($_[$i]) {
            $NQSHash{"table_col_wid$j"} = $_[$i];
        } else {
            $NQSHash{"table_col_wid$j"} = 0;
        }
    }

    my $outstring = "";
    $NQSHash{'table_cols'} = $cols;
    $NQSHash{'table_row'} = -1;
    $NQSHash{'table_cur_col'} = 0;
    $NQSHash{'table_col_type'} = 'standard';
...
sub title_row {
    ...
    $outstring .= "<tr><td bgcolor=$bgcolor colspan=$NQSHash{'table_cols'}><font color=$textcolor><b>$text</b></font></td></tr>\n";
...
sub add_header_row {
    ...
    if ($NQSHash{'table_row'} >= 0) {
        $outstring .= "</td></tr>\n";
        if ($NQSHash{'table_row'} > 0) {
            $outstring .= "<tr><td colspan=$NQSHash{'table_cols'} height=4></td></tr>\n";
        }
    }
...
sub add_row {
    ...
    if ($NQSHash{'table_row'} >= 0) {
        if ($NQSHash{'table_col_type'} eq 'link') {
            $outstring .= "</a>";
        }
...
sub add_col {
    ...
    if ($NQSHash{'table_cur_col'} >= 1) {
        if ($NQSHash{'table_col_type'} eq 'link') {
            $outstring .= "</a>";
        }
        $outstring .= "</b></font></td>";
    }
    $NQSHash{'table_cur_col'}++;
    $outstring .= "<td";
    if ($colspan) {
        $outstring .= " colspan=$colspan";
    } else {
        my $cur_col = $NQSHash{'table_cur_col'};
        my $hashvar = "table_col_wid$cur_col";
        if ($NQSHash{$hashvar} != 0) {
            $outstring .= " width=" . $NQSHash{$hashvar};
        }
    }
...
# routine to add a record link to a table
sub add_record_link {
    my $url_link=$_[0];
...
sub end_table {
    ...
    $outstring .= "</table>";
#    for (my $i = 1; $i<=$NQSHash{'table_cols'); $i++) {
#        $NQSHash{"table_col_wid$i"} = "";
#    }
    $NQSHash{'table_cols'} = "";
    $NQSHash{'table_row'} = "";
    $NQSHash{'table_cur_col'} = "";
    $NQSHash{'table_col_type'} = "";
#    $NQSHash{'table_'} = "";
...
###################################################################################################################################
   ...
   &log_activity($dbh, 'T', $usersid, $appError."\n\n".$oracleError);
   $errorMessage =~ s/\n/\\n/g;
   return ($errorMessage);
}
