#
# $Source: /usr/local/homes/patelr/www/gov.doe.ocrwm.ydappdev/rcs/qa/perl/RCS/UIShared.pm,v $
#
# $Revision: 1.3 $ 
#
# $Date: 2009/01/16 21:10:01 $
#
# $Author: patelr $
#
# $Locker:  $
#
# $Log: UIShared.pm,v $
# Revision 1.3  2009/01/16 21:10:01  patelr
# added attachment code
#
# Revision 1.1  2004/01/13 13:58:19  starkeyj
# Initial revision
#
#
package UIShared;
...
###################################################################################################################################
sub doMultipartHeader {  # routine to generate html page headers
###################################################################################################################################
    my %args = (
        schema => $ENV{SCHEMA},
        title => "$ENV{SCHEMA} Management",
...
###################################################################################################################################
    ...
    $output .= <<END_OF_BLOCK;
       function submitFormCGIResults(script,command) {
           ...
           document.$args{form}.target = 'control';
           document.$args{form}.submit();
       }
       function submitForm(script,command) {
           document.$args{form}.command.value = command;
           document.$args{form}.action = '$args{path}' + script + '.pl';
           document.$args{form}.target = 'workspace';
           document.$args{form}.submit();
       }
...
###################################################################################################################################
    ...
    $output .= "name=$args{form} method=post target=workspace action=$args{formAction}>\n";
