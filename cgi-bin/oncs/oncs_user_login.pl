...
print "<title>Historical Commitment Data</title>\n";
...
if ($cgiaction eq "login")
  ...
  if (($status == 1) || ($status == 2))
    ...
    print<<CMSFrame1;
      <frameset rows=$headerframesize,0,$menuframesize,* frameborder=no frameborder=0 framespacing=0>
...
CMSFrame1
    if ($usersid == 0)
      ...
      {
      print "      <frame src=\"/cgi-bin/oncs/commitment_module_menu.pl?loginusername=$username&loginusersid=$usersid\" name=menu noresize scrolling=no>\n";
      print "      <frame src=\"/cgi-bin/oncs/commitment_module_main.pl?loginusername=$username&loginusersid=$usersid\" noresize name=workspace frameborder=1 frameborder=yes>\n";
      }
    print"    </frameset>\n";
    }
  else
...
if ($cgiaction ne "login")
  ...
  print "<center>\n";
  if ($cgiaction eq "newlogin") {
      print "<h2>Login for Historical Commitment Data</h2><br>\n";
      ...
      print "<h2>Historical Commitment Data</h2><br>\n";
      print "<input type=hidden name=reportname value=custom>\n";
      print "<input type=hidden name=username value=GUEST>\n";
      print "<input type=hidden name=password value=GUEST>\n";
      print "<br><input type=button name=dologin value='Login For Edits' onclick=\"document.login.cgiaction.value='newlogin';document.login.submit();\">\n";
      print "<br><br><br><input type=button name=guest value='Managers Report' onclick=\"document.login.cgiaction.value='login';document.login.reportname.value='managers';document.login.submit();\">\n";
      print "<br><br><br><input type=button name=adhocbutton value='Customizable Report' onclick=\"document.login.cgiaction.value='login';document.login.reportname.value='custom';document.login.submit();\">\n";
  }
