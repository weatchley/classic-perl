...
#  $changerequestnum = $commitmenthash{'changerequestnum'};
  ...
  my $wbsdescription;
  ($wbsdescription) = lookup_column_values($dbh, "workbreakdownstructure", "description", "controlaccountid = '$controlaccountid'");
  $wbsdescription = getDisplayString($wbsdescription, 80);
...
#  my $mocmgrroleid = lookup_role_by_name($dbh, "M&O Commitment Manager");
  ...
  <tr>
    ...
    <td>
    $controlaccountid - $wbsdescription
    </td>
  </tr>
  <tr>
    <td align=center>
...
#  $resubmitrationale = $testout->param('resubmitrationale');
  ...
  my $controlaccountid = "'" . $testout->param('workbreakdownstructure') . "'";
  #$thiswbs =~ m/ \. /;
  #$changerequestnum = "'" . $` . "'";
  #$controlaccountid = "'" . $' . "'";
...
#    }
  my $sqlstring = "UPDATE $SCHEMA.$updatetable SET duedate=$duedate, statusid=$statusid,
                 ...
                 closeddate=$closeddate, controlaccountid=$controlaccountid, issueid=$issueid, approver=$approver,
...
#  $changerequestnum = $commitmenthash{'changerequestnum'};
  $controlaccountid = $commitmenthash{'controlaccountid'};
  $wbsavailchk = ($controlaccountid eq "") ? "checked" : "";
  $issueid = $commitmenthash{'issueid'};
  $approver = $commitmenthash{'approver'};
  $replacedby = $commitmenthash{'replacedby'};
  $updatedby = $commitmenthash{'updatedby'};
...
committable4
  foreach $key (sort keys %wbshash)
    {
    my $wbsdisplaystring = getDisplayString($wbshash{$key}, 80);
    my $selectedstring = ("$controlaccountid" eq $key) ? "selected" : "";
    print "    <option value=\"$key\" $selectedstring>$key -- $wbsdisplaystring\n";
    }
  print <<committable5;
    </select>
    </td>
