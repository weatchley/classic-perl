...
#
# $Source: /data/dev/rcs/pcl/perl/RCS/UIUsers.pm,v $
#
# $Revision: 1.9 $
#
# $Date: 2004/05/21 19:26:36 $
#
# $Author: munroeb $
#
# $Locker:  $
#
# $Log: UIUsers.pm,v $
# Revision 1.9  2004/05/21 19:26:36  munroeb
# modified to reflect cybersecurity password change issue
#
# Revision 1.8  2003/02/25 19:00:23  atchleyb
# fixed bug that would report the wrong error when a users is changing thier password
#
# Revision 1.7  2003/02/21 18:01:30  atchleyb
# fixed bug in testpassword with a regulare expression
#
# Revision 1.6  2003/02/14 18:17:19  atchleyb
# added improved password security
#
# Revision 1.5  2003/02/12 18:53:35  atchleyb
# added session management
#
# Revision 1.4  2003/02/03 20:25:02  atchleyb
# removed refs to SCM
#
# Revision 1.3  2002/12/02 17:21:35  atchleyb
# added prompt boxes to links
# added status to browse user table
#
...
###################################################################################################################################
   ...
   if (($args{command} eq "adduser") || ($args{command} eq "adduserform")) {
      ...
      $title = "Update User";
   } elsif (($args{command} eq "browse") || (($args{command} eq "displayuser")) || (($args{command} eq "displayuser_by_unixid")) || ($args{command} eq "displayuserform")) {
      $title = "Browse User";
...
###################################################################################################################################
    ...
    my %valueHash = (
       ...
       sccbid => (defined($mycgi->param("sccbid"))) ? $mycgi->param("sccbid") : "",
...
###################################################################################################################################
    ...
    $output .= "<br><br>\n</td></tr></table>\n";
    $extraHTML .= "<form name=dummy$form>\n";
    ...
    $extraHTML .= "<input type=hidden name=server value=$Server>\n";
...
###################################################################################################################################
    ...
    $output .= &writeUserTable(dbh => $args{dbh}, schema => $args{schema}, privilege => 9, title => 'SCCB Members', titleBackground => '#ffc0ff');
...
###################################################################################################################################
sub doUserEntryForm {  # routine to generate a user date entry/update form
###################################################################################################################################
    my %args = (
        type => 'new',
        @_,
    ...
    $output .= "<tr><td colspan=4><br></td></tr>\n";
    $output .= "<tr><td><b>Organization: </b>&nbsp</td><td colspan=3><input type=text name=organization value=\"$userInfo{organization}\" maxlength=75 size=40></td>\n";
    $output .= "<tr><td colspan=4><br></td></tr>\n";
    ...
    if (&doesUserHavePriv(dbh => $args{dbh}, schema => $args{schema}, userid => $args{userID}, privList => [-1])) {
        $output .= "<tr><td><b>SCCB User ID: </b>&nbsp</td><td colspan=3><input type=text name=\"sccbid\" maxlength=5 size=5 value=\" " . ((defined ($userInfo{sccbid}) && $userInfo{sccbid} > 0) ? $userInfo{sccbid} : "") . "\"></td></tr>\n";
        $output .= "<tr><td colspan=4><br></td></tr>\n";
    }
    ...
    $output .= "</table></td></tr>\n";
    $output .= "<tr><td colspan=4>&nbsp;<hr></td></tr>\n";
    $output .= "<tr><td colspan=4 align=center><input type=button name=submitbutton value=\"Submit User Information\" onClick=\"verifySubmit(document.$args{form})\"> &nbsp;\n";
    if ($args{type} eq 'update') {
...
###################################################################################################################################
    ...
    if (index(uc($args{password}), uc($args{username})) >=0) {$acceptable = 'F';} # can not include username
    if (index(uc($args{password}), uc(reverse($args{username}))) >=0) {$acceptable = 'F';} # can not include reverse of username
    foreach my $val (@commonStrings) {
        if (index(uc($args{password}), uc($val)) >=0) {$acceptable = 'F';} # can not include a common string
        if (index(uc($args{password}), uc(reverse($val))) >=0) {$acceptable = 'F';} # can not include reverse of common string
    }
    if ($args{password} =~ /^\d|\d$/) {$acceptable = 'F';} # can not start or end with a digit
    my $passCount = 0;
