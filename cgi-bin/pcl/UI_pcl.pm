#
# $Source: /data/dev/rcs/pcl/perl/RCS/UI_pcl.pm,v $
#
# $Revision: 1.11 $ 
#
# $Date: 2003/01/27 23:24:35 $
#
# $Author: atchleyb $
#
# $Locker:  $
...
###################################################################################################################################
    ...
    foreach my $item ($args{valueList}) {
        $valueHash{$item} = $pclcgi->param($item);
    }
...
###################################################################################################################################
    ...
    my $fileContents = '';
    my $name = $pclcgi->param($args{fileParam});
    my $buffer = '';
    my $bytesread = 0;
...
###################################################################################################################################
   ...
   return ($pclcgi->header($args{mimetype}));
}
...
###################################################################################################################################
    ...
    $output .= &doStartForm(schema => $args{schema}, form => $args{form}, useFileUpload => $args{useFileUpload}, formAction => $args{formAction}, 
              onSubmit => $args{onSubmit}, username => $settings{username}, userid => $settings{userid});
...
###################################################################################################################################
    ...
    $output .= "<body text=#000099 background=$PCLImagePath/background.gif>\n";
