...
use vars qw($DBUser $SCMConnectPath $SCMServer);
...
$DBUser = $ENV{'DBUser'};
$SCMConnectPath = $ENV{'SCMConnectPath'};
$SCMServer = $ENV{'SCMServer'};
#$schema = $ENV{'SCHEMA'};
...
@EXPORT = qw (&db_connect                &db_disconnect   
              ...
              &get_userid                &get_username 
              &get_fullname              &get_next_commentor_id 
              &get_next_users_id         &get_lookup_values
              &get_value                 &get_authorized_users
              &get_date                  &log_activity
              &log_error                 &db_encrypt_password
              &checkLogin                
);
@EXPORT_OK = qw(&db_connect                 &db_disconnect 
                &validate_user              &does_user_have_priv 
                &get_userid                 &get_username 
                &get_fullname               &get_next_commentor_id 
                &get_next_users_id          &get_lookup_values 
...
%EXPORT_TAGS =(
    Functions => [qw(&db_connect                   &db_disconnect 
                     &validate_user                &does_user_have_priv 
                     &get_userid                   &get_username 
                     &get_fullname                 &get_next_commentor_id 
                     &get_next_users_id            &get_lookup_values 
...
###################
    ...
    my $sqlquery = "select password from $schema.scruser where (username = '$username') and (password = '$password') and (isactive = 'T')";
...
#########################
    ...
    if ($privtype eq "number") {
        $sqlquery = "SELECT privilegeid FROM $schema.scruserprivilege WHERE (userid = $userid) and (privilegeid IN $privlist)";
    } else {
        $sqlquery = "SELECT description FROM $schema.scruserprivilege up, $schema.scrprivilege sp WHERE (up.userid = $userid) and (sp.description IN $privlist)";
    }
...
################
    ...
    my @values;
    $username = uc($username);
    my $sqlquery = "select id from $schema.scruser where username = '$username'";
    my $csr = $dbh->prepare($sqlquery);
        $csr->execute;
        @values = $csr->fetchrow_array;
        $csr->finish;
...
##################
    ...
    my $csr;
    my @values;
    my $sqlquery = "select username from $schema.scruser where id = $userid";
    $csr = $dbh->prepare($sqlquery);
    $csr->execute;
    @values = $csr->fetchrow_array;
    $csr->finish;
...
##################
    ...
    my @values;
    my $fullname;
    my $sqlquery = "select firstname, lastname from $schema.scruser where id = $userid";
        $csr = $dbh->prepare($sqlquery);
        $csr->execute;
        @values = $csr->fetchrow_array;
        $csr->finish;
...
#######################
    ...
    my $status = 1;
    my $logdate = &get_date();
    my $sqlquery = "INSERT INTO $schema.activity_log (userid, datelogged, iserror, text) VALUES ($userid, SYSDATE, '$errorStatus', '$logmessage')";
    eval {
        $dbh->do($sqlquery);
        $dbh->commit;
    };
...
#  redirect user to login screen if username, userid, or schema are 
#  not defined (i.e. user hasn't logged in)
################
sub checkLogin {
################
