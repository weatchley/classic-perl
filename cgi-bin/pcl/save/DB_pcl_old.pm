...
#
# $Source: /data/dev/rcs/pcl/perl/RCS/DB_pcl.pm,v $
# $Revision: 1.17 $
# $Date: 2003/01/28 00:10:21 $
#
# $Author: atchleyb $
# $Locker:  $
#
...
use strict;
use Carp;
use PCL_Header qw(:Constants);
use vars qw(@ISA @EXPORT @EXPORT_OK %EXPORT_TAGS $VERSION $AUTOLOAD);
use vars qw($DBUser $PCLConnectPath $PCLServer);
use Tie::IxHash;
use DBI;
use DBD::Oracle qw(:ora_types);

$DBUser = $ENV{'DBUser'};
$PCLConnectPath = $ENV{'PCLConnectPath'};
$PCLServer = $ENV{'PCLServer'};
...
################################################################################################################
    ...
    if (open (FH, "$PCLConnectPath |")) {
       ($temp, $password) = split('//', <FH>);
        chomp($password);
        $password = decrypt(text => $password, key => $PCLPassKey);
        close (FH);
    } else {
        $username = "null";
        $password = "null";
...
################################################################################################################
    ...
    if ($@) {
        print STDERR "\nDB_scm.pm/db_connect - Error Message: $@\n";
    }
    return ($dbh);
}
...
################################################################################################################
    ...
    if ($@) {
        print STDERR "\nDB_scm.pm/db_disconnect - Error Message: $@\n";
    }
    return ($rc);
}
...
################################################################################################################
    ...
    if ($@) {
        ...
        print STDERR "\nDB_scm.pm/log_error - Passed Message: $logmessage\n - Error Message: \"$@\"\n";
    } else {
		$dbh->commit;
	 }
    return ($status);
