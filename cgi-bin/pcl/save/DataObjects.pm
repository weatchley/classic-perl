...
my ($scmcgi, $path, $form, $instructionsColor, $secondReviewName);
BEGIN {
   $scmcgi = new CGI;
   $ENV{SCRIPT_NAME} =~ m%(.*/)(.*)\..*$%;
   $path = $1;
   $form = $2;
   $instructionsColor = $SCMFontColor;
...
########################
   ...
   my $where;
   my $changeImpact = $scmcgi->param("$args{changeImpactName}");
   my $changeControlNumber = $scmcgi->param("$args{changeControlName}");
...
#######################
   my %args = (
      useForm => defined($scmcgi->param("useFormValues")) ? 1 : 0,
      changeImpactName => 'changeImpact',
      changeControlName => 'changeControlNumber',
   ...
   if ($args{useForm}) {
      $changeImpact = $scmcgi->param($args{changeImpactName});
      $changeControl = (defined($scmcgi->param($args{changeControlName}))) ? $scmcgi->param($args{changeControlName}) : "";
   } elsif ($args{doSelect}) {
      if (($args{table} eq 'comments') || ($args{table} eq 'comments_entry')) {
...
###################
   ...
   my $dateDue = "NULL";
   my ($day, $month, $year) = ($scmcgi->param($args{name} . "_day"), $scmcgi->param($args{name} . "_month"), $scmcgi->param($args{name} . "_year"));
...
##################
   ...
   if ($args{useForm}) {
      ...
      $date = $scmcgi->param($month) if (defined($scmcgi->param($month)));
      $date .= "/";
      $date .= $scmcgi->param($day) if (defined($scmcgi->param($day)));
      $date .= "/";
      $date .= $scmcgi->param($year) if (defined($scmcgi->param($year)));
   } elsif ($args{doSelect}) {
...
##########################
   ...
   my $where;
   my $hasCommitments = $scmcgi->param("$args{name}");
...
#########################
   ...
   my $checked = "";
   if ($args{useForm}) {
      $checked = (defined($scmcgi->param($args{name}))) ? "checked" : "";
   } elsif ($args{doSelect}) {
      my $hasCommitments;
      if (($args{table} eq 'comments') || ($args{table} eq 'comments_entry')) {
...
#####################
   ...
   my $where;
   my $hasIssues = $scmcgi->param("$args{name}");
...
#######################
   ...
   my $reviewer = (defined($scmcgi->param("$args{selectedName}"))) ? $scmcgi->param("$args{reviewerIDName}") : 'NULL';
...
######################
   ...
   if ($args{useForm}) {
      $reviewerID = $scmcgi->param($args{reviewerIDName});
      $isChecked = (defined($scmcgi->param($args{selectedName}))) ? 'checked' : '';
   } elsif ($args{doSelect}) {
...
##########################
   ...
   my $writer = $scmcgi->param("$args{name}");
...
#########################
   ...
   my $current = ($args{useForm}) ? $scmcgi->param($args{name}) : $writer;
...
########################
   ...
   foreach my $id (keys (%reviewers)) {
      ...
      if ($args{useForm}) {
         $checked = (defined($scmcgi->param($name))) ? "checked" : "";
      } else {
