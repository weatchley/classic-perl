...
#################
   my %args = (
	...
	my $numColumns = 5;
	$outstring .= &startTable(columns => $numColumns, title => "$roleArray[0]{sccbname}", width => 700, border=>0,titleBackground => "#f0f0f0");
	...
	$outstring .= &addSpacerRow (columns => $numColumns);
	$outstring .= &startRow (bgColor => "#f0f0f0");
	$outstring .= &addCol (value => "<font color=black size=-1><b>Roles</b></font>");
	$outstring .= &addCol (value => "<font color=black size=-1><b>Primary</b></font>");
	$outstring .= &addCol (value => "<font color=black size=-1><b>Alternate</b></font>",colspan=>3);
	$outstring .= &endRow();
	for (my $i = 0; $i < $#roleArray; $i++) {
		$outstring .= &startRow;
		$outstring .= &addCol (value => "$roleArray[$i]{desc}",width=>300);
		$rolenames = "$roleArray[$i]{name},";
		while ($roleArray[$i]{desc} eq $roleArray[$i+1]{desc} && $roleArray[$i+1]{isalt} eq 'F') {
			$rolenames .= "&nbsp;$roleArray[++$i]{name},";
		}
		chop($rolenames);
		$outstring .= &addCol (value => "<font size=-1>$rolenames</font>");
		$rolenames = "$roleArray[++$i]{name},";
		while ($roleArray[$i]{desc} eq $roleArray[$i+1]{desc} && $roleArray[$i+1]{isalt} eq 'T') {
			$rolenames .= "&nbsp;$roleArray[++$i]{name},";
		}
		chop($rolenames);
		$outstring .= &addCol (value => "<font size=-1>$rolenames</font>");
		#$outstring .= "TBD";
		$outstring .= &addCol (value => "<a href=javascript:alert('edit');>Edit</a>&nbsp;&nbsp;</b></font>");
		$outstring .= &addCol (value => "<a href=javascript:alert('delete');>Delete</a>&nbsp;&nbsp;</b></font>");
		$outstring .= &endRow();
	}
	$outstring .= &endTable;
