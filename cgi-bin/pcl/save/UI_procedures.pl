...
#
# $Source: /data/dev/rcs/scm/perl/RCS/UI_procedures.pl,v $
#
# $Revision: 1.1 $
#
# $Date: 2002/09/17 20:14:48 $
#
# $Author: atchleyb $
#
# $Locker:  $
#
# $Log: UI_procedures.pl,v $
# Revision 1.1  2002/09/17 20:14:48  atchleyb
# Initial revision
#
#
#
...
###################################################################################################################################
    my %args = (
        title => 'Documented Procedure Management',
        displayTitle => 'T',
        @_,
    );
    my $output = "";
...
if ($command eq "browse") {
    print &doHeader(title => 'Browse Documented Procedures');
    eval {
        print &doBrowseDocumentTable(dbh => $dbh, schema => $schema, itemType => $itemType, title => 'Documented Procedures', userID => $userid);
...
###################################################################################################################################
    ...
    print &doHeader(title => 'Add Documented Procedure');
    eval {
        print &doAddDocumentForm(dbh => $dbh, schema => $schema, itemType => $itemType, title => 'Documented Procedures', form => $form, userID => $userid);
    };
    if ($@) {
...
#print "File:\n" . length($fileContents) . "\nEnd File\n";
        print &doProcessAddDocument(dbh => $dbh, schema => $schema, itemType => $itemType, title => 'Documented Procedures', form => $form, 
...
###################################################################################################################################
    ...
    print &doHeader(title => 'Update Documented Procedure');
    eval {
        print &doBrowseDocumentTable(dbh => $dbh, schema => $schema, itemType => $itemType, title => 'Documented Procedures', update => 'T', userID => $userid);
    };
    if ($@) {
...
###################################################################################################################################
    ...
    eval {
        print &doUpdateDocumentInfoForm(dbh => $dbh, schema => $schema, document => $scmcgi->param('document'), 
              title => 'Documented Procedures', form => $form, userID => $userid);
    };
    if ($@) {
...
###################################################################################################################################
    ...
    eval {
        print &doProcessUpdateDocumentInfo(dbh => $dbh, schema => $schema, document => $scmcgi->param('itemid'), 
              name => $scmcgi->param('name'), description => $scmcgi->param('description'), title => 'Documented Procedures', form => $form, userID => $userid);
    };
    if ($@) {
...
###################################################################################################################################
    ...
    eval {
        print &doUpdateDocumentForm(dbh => $dbh, schema => $schema, document => $scmcgi->param('document'), 
              itemType => $itemType, title => 'Documented Procedures', form => $form, userID => $userid);
    };
    if ($@) {
...
#print "File:\n" . length($fileContents) . "\nEnd File\n";
        print &doProcessUpdateDocument(dbh => $dbh, schema => $schema, itemID => $scmcgi->param('itemid'), title => 'Documented Procedures', form => $form, 
              file => $fileContents, fileName => $name, majorVersion => $scmcgi->param('major'), minorVersion => $scmcgi->param('minor'), 
              description => $scmcgi->param('description'), userID => $userid, userName => $username);
...
###################################################################################################################################
    ...
    eval {
        my $fileContents = '';
        my $name = $scmcgi->param('documentfile');
        print &doProcessCheckInNoChange(dbh => $dbh, schema => $schema, itemID => $scmcgi->param('itemid'), title => 'Documented Procedures', form => $form, 
