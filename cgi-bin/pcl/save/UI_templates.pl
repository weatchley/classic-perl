...
#
# $Source: /data/dev/rcs/scm/perl/RCS/UI_templates.pl,v $
#
# $Revision: 1.1 $
#
# $Date: 2002/09/17 20:16:30 $
#
# $Author: atchleyb $
#
# $Locker:  $
#
# $Log: UI_templates.pl,v $
# Revision 1.1  2002/09/17 20:16:30  atchleyb
# Initial revision
#
#
#
...
if ($command eq "browse") {
    print &doHeader(title => 'Browse Templates');
    eval {
        print &doBrowseDocumentTable(dbh => $dbh, schema => $schema, itemType => $itemType, title => 'Templates', userID => $userid);
...
###################################################################################################################################
    ...
    eval {
        print &doDisplayDocument(dbh => $dbh, schema => $schema, itemType => $itemType, document => $scmcgi->param('document'),
                majorVersion => $scmcgi->param('majorversion'), minorVersion => $scmcgi->param('minorversion'), userID => $userid);
    };
    if ($@) {
...
###################################################################################################################################
    ...
    print &doHeader(title => 'Add Template');
    eval {
        print &doAddDocumentForm(dbh => $dbh, schema => $schema, itemType => $itemType, title => 'Templates', form => $form, userID => $userid);
    };
    if ($@) {
...
#print "File:\n" . length($fileContents) . "\nEnd File\n";
        print &doProcessAddDocument(dbh => $dbh, schema => $schema, itemType => $itemType, title => 'Templates', form => $form, 
...
###################################################################################################################################
    ...
    print &doHeader(title => 'Update Template');
    eval {
        print &doBrowseDocumentTable(dbh => $dbh, schema => $schema, itemType => $itemType, title => 'Templates', update => 'T', userID => $userid);
    };
    if ($@) {
...
###################################################################################################################################
    ...
    eval {
        print &doUpdateDocumentInfoForm(dbh => $dbh, schema => $schema, document => $scmcgi->param('document'), 
              title => 'Templates', form => $form, userID => $userid);
    };
    if ($@) {
...
###################################################################################################################################
    ...
    eval {
        print &doProcessUpdateDocumentInfo(dbh => $dbh, schema => $schema, document => $scmcgi->param('itemid'), 
              name => $scmcgi->param('name'), description => $scmcgi->param('description'), title => 'Templates', form => $form, userID => $userid);
    };
    if ($@) {
...
###################################################################################################################################
    ...
    eval {
        print &doUpdateDocumentForm(dbh => $dbh, schema => $schema, document => $scmcgi->param('document'), 
              itemType => $itemType, title => 'Templates', form => $form, userID => $userid);
    };
    if ($@) {
...
#print "File:\n" . length($fileContents) . "\nEnd File\n";
        print &doProcessUpdateDocument(dbh => $dbh, schema => $schema, itemID => $scmcgi->param('itemid'), title => 'Templates', form => $form, 
              file => $fileContents, fileName => $name, majorVersion => $scmcgi->param('major'), minorVersion => $scmcgi->param('minor'), 
              description => $scmcgi->param('description'), userID => $userid, userName => $username);
...
###################################################################################################################################
    ...
    eval {
        my $fileContents = '';
        my $name = $scmcgi->param('documentfile');
        print &doProcessCheckInNoChange(dbh => $dbh, schema => $schema, itemID => $scmcgi->param('itemid'), title => 'Templates', form => $form, 
