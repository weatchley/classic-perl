...
#
# $Source: /data/dev/rcs/plaqads/perl/RCS/DBShared.pm,v $
# $Revision: 1.2 $
# $Date: 2005/04/04 16:15:24 $
#
# $Author: atchleyb $
# $Locker:  $
#
# $Log: DBShared.pm,v $
# Revision 1.2  2005/04/04 16:15:24  atchleyb
# updated to fix problem with single quotes in log activity
#
...
###################################################################################################################################
   ...
   my $orderBy = ($args{orderBy}) ? "ORDER BY $args{orderBy}" : "";
   my $where = ($args{where}) ? "WHERE $args{where}" : "";
   my $csr = $args{dbh}->prepare("SELECT $args{idColumn}, $args{nameColumn} FROM $args{schema}.$args{table} $where $orderBy");
   $csr->execute;
   my $i = 0;
   while (my @values = $csr->fetchrow_array) {
      $lookup[$i]{id} = $values[0];
      $lookup[$i]{value} = $values[1];
      $i++;
   }
   $csr->finish;
   return (@lookup);
