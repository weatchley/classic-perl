#
# $Source: /data/dev/rcs/plaqads/perl/RCS/UIShared.pm,v $
#
# $Revision: 1.1 $ 
#
...
@EXPORT_OK = qw(
      ...
      &writeHTMLHead          &checkLogin    &writeAlert            &getStandardValues
...
###################################################################################################################################
sub getStandardValues {  # routine to get initial CGI values and return in a hash
###################################################################################################################################
    my %args = (
        @_,
    );
...
###################################################################################################################################
    ...
    $output .= &doStartPage(dbh => $args{dbh}, schema => $args{schema}, title => $args{title}, displayTitle => $args{displayTitle}, 
...
###################################################################################################################################
    ...
    if ($args{includeJSCalendar} eq 'T') {
        $output .= "    <script language=\"javascript\" src=\"$SYSJavaScriptPath/cal.js\"></script>\n";
        $output .= "    <script language=\"javascript\" src=\"$SYSJavaScriptPath/cal_conf.js\"></script>\n";
    }
...
###################################################################################################################################
    ...
    $output .= "<input type=hidden name=nullhiddenvalueuserid value=$args{userid}>\n";
