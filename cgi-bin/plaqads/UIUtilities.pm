...
#
# $Source: /data/dev/rcs/plaqads/perl/RCS/UIUtilities.pm,v $
#
# $Revision: 1.2 $
#
# $Date: 2004/12/02 18:36:09 $
#
# $Author: atchleyb $
#
# $Locker:  $
...
# Revision 1.2  2004/12/02 18:36:09  atchleyb
# added menu miscellaneous with keyword and category maint links
#
...
# User menu
    ...
    $userMenu->addMenu(name => 'updateroles', label => 'Update User Roles', contents => "javascript:submitForm('roles','updateuserselect');");
    my @users = &getUserArray(dbh => $args{dbh}, schema=>$args{schema});
    my $text = "<select name=userrolereviewid size=1><option value=0 selected>Select a user</option>\n";
...
# Miscellaneous menu
    ...
    $miscMenu->addMenu(name => 'updatekeywords', label => 'Update Keywords', contents => "javascript:submitForm('keywords','updatekeywordselect');");
    $miscMenu->addMenu(name => 'updatecategories', label => 'Update Categories', contents => "javascript:submitForm('categories','updatecategoryselect');");
...
# Top menu
    ...
    if (&doesUserHavePriv(dbh=>$args{dbh}, schema=>$args{schema}, userid=>$args{userID}, privList=>[10])) {
        ...
        $menu1->addMenu(name => "misc", label => "Miscellaneous", contents => $miscMenu->buildMenus(name => 'misc', type => $subMenuType), title => 'Miscellaneous Utilities');
