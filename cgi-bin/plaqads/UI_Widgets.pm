...
#
# $Source: /data/dev/rcs/plaqads/perl/RCS/UI_Widgets.pm,v $
#
# $Revision: 1.2 $
#
# $Date: 2004/11/16 19:36:22 $
#
# $Author: atchleyb $
#
# $Locker:  $
#
# $Log: UI_Widgets.pm,v $
# Revision 1.2  2004/11/16 19:36:22  atchleyb
# added vertical form of dual select
#
...
@EXPORT = qw (
   ...
   &doAlertBox            &buildSectionBlock           &build_dual_select_vertical
);
@EXPORT_OK = qw(
   &build_dual_select     &build_select_from_piclist   &build_drop_box 
   &build_date_selection  &build_radio_block_wide      &gen_table 
...
####################################################################################################################################
sub build_dual_select_vertical {
# routine to build a dual selection box from hashes passed to it
####################################################################################################################################
    my $elementname=$_[0];
    my $formname = $_[1];
...
# remove entrys from left table that exist in right table
    ...
    $outstring .= "<!-- the select and deselect buttons call the javascript to move selected element to twin option box -->\n";
    $outstring .= "<input type=button value=\"\\/\" name=select$elementname  onClick=\"process_dual_select_option(document.$formname.avail$elementname,document.$formname.$elementname,'move')\"> &nbsp; \n";
    $outstring .= "<input type=button value=\"/\\\" name=deselect$elementname  onClick=\"process_dual_select_option(document.$formname.$elementname,document.$formname.avail$elementname,'move'";
    for (my $i=0; $i<=$#lockedvals; $i++) {
        $outstring .= ", '$lockedvals[$i]'";
    }
    $outstring .= ")\"><br>\n";
    $outstring .= "</td></tr><tr><td valign=top>\n";
    $outstring .= "$rightname<br>\n";
    $outstring .= "<!-- the ondblclick event calls the javascript to move selected element to twin option box -->\n";
    $outstring .= "<select multiple name=$elementname size=5 ondblclick=\"process_dual_select_option(document.$formname.$elementname,document.$formname.avail$elementname,'move'";
    for (my $i=0; $i<=$#lockedvals; $i++) {
