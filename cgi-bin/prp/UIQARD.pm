...
#
# $Source: /usr/local/homes/higashis/www/gov.doe.ocrwm.ydappdev/rcs/prp/perl/RCS/UIQARD.pm,v $
# $Revision: 1.33 $
...
#
# $Log: UIQARD.pm,v $
# Revision 1.33  2007/11/27 00:58:38  higashis
# change request implementation item a, b, c (the first try)
#
# Revision 1.32  2007/11/20 19:16:44  higashis
# changes for creq00084 c)
#
# Revision 1.31  2006/06/22 19:24:47  naydenoa
# CREQ00081 - remove color-coding. Commented out coloring button in section browse subroutine.
#
# Revision 1.30  2006/06/13 23:43:55  naydenoa
# CREQ00077 - allow effective dates in future for QARD revs.
#
# Revision 1.29  2006/04/06 22:43:21  naydenoa
# CREQ00076 - add columns to undelete display (undeleteselect function)
#
# Revision 1.28  2006/01/10 23:46:55  naydenoa
# CREQ00072, CREQ00073
# Add images for figures, take out zeroes.
#
# Revision 1.27  2005/12/09 22:56:48  naydenoa
# Changed ORD to OCRWM where applicable.
#
...
//        document.$form.sid.value = sectionid;
    ...
    function displayOtherImage (revisionid, qrid) {
        var myDate = new Date();
        var winName = myDate.getTime();
        document.$form.command.value = 'view_other_image';
        document.$form.rid.value = revisionid;
        document.$form.qrid.value = qrid;
        document.$form.action = '$path$form.pl';
        document.$form.target = winName;
        var newwin = window.open('',winName);
        newwin.creator = self;
...
#print STDERR "$matrixid\n";
    ...
    for (my $i = 1; $i <= $#sections; $i++) {
        ...
        my $titleaddimage = "";
        if ($sectionid eq "FIGURE 1" || $sectionid eq "FIGURE 2") {
            $titleaddimage = "&nbsp;&nbsp;(<a href=javascript:displayOtherImage('',$qrid) title=\"Click for $sectionid image\">Image</a>)";
	}
        $title = ($sections[$i]{title}) ? "<b>$sections[$i]{title}</b>$titleaddimage$br" : "";
...
#################
    ...
    if ($settings{command} eq "pop_table") {
        ...
        $outstr .= "<center><font face=helvetica><b>$qardtype $revision<br></center>Table 1, Item $item - $subid</b></center><br><br><font size=-1 face=times><b>US NRC Document: </b><br>$nrcdescription<br><br><b>National/Industry Standard:</b><br>$standarddescription<br><br><b>OCRWM Position:</b><br>$position</font>\n";
...
#    my $isupdate = ($settings{rid} || $settings{isupdate}) ? 1 : 0;
    ...
    my $dselected = ($status eq "D") ? " selected" : "";
    my $aselected = ($status eq "A") ? " selected" : "";
    my $rselected = ($status eq "R") ? " selected" : "";
    $outstr .= "<tr><td><li><b>Revision Status:</b></td><td><select name=status><option value=D$dselected>Draft<option value=A$aselected>Approved<option value=R$rselected>Archived</select></td>\n";
...
###
    ...
    function validateStuff(command) {
        ...
        msg += (document.$args{form}.dateeffective_year.value != "") ? (validate_date(document.$args{form}.dateeffective_year.value,document.$args{form}.dateeffective_month.value,document.$args{form}.dateeffective_day.value,0,0,0,0,1,1,0)) : "";
	
        if (msg != "") {
            alert (msg);
        }
        else {
		document.$args{form}.iscurrent.disabled = false;
		submitFormCGIResults('$args{form}','enter_qard_process');
		document.$args{form}.iscurrent.disabled = true;
        }
    }
    
    if(document.$args{form}.status.value=="A"){try{window.onload = disableform()}catch(e){}}
    
    function disableform(){
	document.$args{form}.revid.readOnly = true;
	document.$args{form}.revid.style.color = "aca899";
	document.$args{form}.revid.onclick = function() {alert('You cannot edit this field while the document is in "approved" status');};
	document.$args{form}.approver.disabled = true; 
	document.$args{form}.dateapproved_year.disabled = true;
	document.$args{form}.dateapproved_month.disabled = true;
	document.$args{form}.dateapproved_day.disabled = true;
	document.$args{form}.dateeffective_year.disabled = true;
	document.$args{form}.dateeffective_month.disabled = true;
	document.$args{form}.dateeffective_day.disabled = true;
	document.$args{form}.isdeleted.onclick = function() {alert('You cannot edit this field while the document is in "approved" status');return false;};
	document.$args{form}.iscurrent.disabled = true;
    }
...
####################
    ...
    if ($settings{selection} eq "section") {
        ...
        @updateselect = &getSections (dbh => $args{dbh}, schema => $args{schema}, where => "isdeleted = 'F' $where", orderby => "tocid, sorter, sectionid, subid, id");
    ...
    if ($settings{selection} eq "section") {
        for (my $i = 1; $i <= $#updateselect; $i++) {
            my $id = $updateselect[$i]{id};
            my $sectionid = $updateselect[$i]{sectionid};
            my $subid = $updateselect[$i]{subid} ? " - $updateselect[$i]{subid}" : "";
            my $title = $updateselect[$i]{title} ? " - $updateselect[$i]{title}" : "";
            my $text = $updateselect[$i]{text} ? " - $updateselect[$i]{text}" : "";
            my $fullsection = "$sectionid$subid$title$text";
            $outstr .= "<option value=$id>" . &getDisplayString ($fullsection, 100) . "\n";
        }
    }  
    elsif ($settings{selection} eq "toc") {
...
##################
    ...
    my $qrid = $settings{qrid} ? $settings{qrid} : 0;

    my $outstr = "";
    my ($mimetype, $image) = &getSingleRow (dbh => $args{dbh}, schema => $args{schema}, table => "qardattachment", what => "imagecontenttype, document", where => "revisionid = $rid or id = $qrid");
    $outstr = "Content-type: $mimetype\n\n";
    $outstr .= $image;
    return ($outstr);
}
...
######################
    ...
    $outstr .= "<tr bgcolor=#a0e0c0><td width=100><b>Undelete</b></td><td><b>" . uc ($args{what}) . "</b></td><td width=80><b>Date&nbsp;Deleted</td><td width=100><b>Deleted By</td><tr>\n";

    my $stuff;
    if ($args{what} eq "section") {
        $stuff = "select 'QARD ' || q.revid || ', Section ' || s.sectionid || ' - ' || '<b>' || s.title || '</b> ' || s.text, s.id, s.lastupdated, u.firstname || ' ' || u.lastname from $args{schema}.qardsection s, $args{schema}.qard q, $args{schema}.users u where s.isdeleted = 'T' and s.qardrevid = q.id and s.updatedby = u.id order by s.qardrevid, s.tocid, s.sorter, s.sectionid, s.lastupdated";
    }
    elsif ($args{what} eq "table1a") {
        $stuff = "select 'QARD ' || q.revid || ', Table 1A Row ' || t.item || ' ' || t.subid, t.id, t.lastupdated, u.firstname || ' ' || u.lastname from $args{schema}.qardtable1a t, $args{schema}.qard q, $args{schema}.users u where t.isdeleted = 'T' and t.revisionid = q.id and t.updatedby = u.id order by t.revisionid, t.item, t.subid";
    }
    elsif ($args{what} eq "toc") {
        $stuff = "select t.tocid || ' ' || t.title, t.id, t.lastupdated, u.firstname || ' ' || u.lastname from $args{schema}.qardtoc t, $args{schema}.users u where t.isdeleted = 'T' and t.updatedby = u.id order by t.title";
    }
    elsif ($args{what} eq "qard") {
        $stuff = "select r.revid, r.id, r.lastupdated, u.firstname || ' ' || u.lastname from $args{schema}.qard r, $args{schema}.users u where r.isdeleted = 'T' and r.qardtypeid = 1 and r.updatedby = u.id";
    }
    elsif ($args{what} eq "aqap") {
        $stuff = "select r.revid, r.id, r.lastupdated, u.firstname || ' ' || u.lastname from $args{schema}.qard r, $args{schema}.users u where r.isdeleted = 'T' and r.qardtypeid = 2 and r.updatedby = u.id";
    }
    my $csr = $args{dbh} -> prepare ($stuff);
    $csr -> execute;
    while (my ($title, $id, $dateup, $upby) = $csr -> fetchrow_array) {
        $outstr .= "<tr valign=top><td width=100><font size=2 face=$SYSFontFace><a href=javascript:submitUndelete('qard','undelete_process','$args{what}',$id)>Undelete</a></td><td><font size=2 face=$SYSFontFace>$title</td><td><font size=2 face=$SYSFontFace>$dateup</td><td><font size=2 face=$SYSFontFace>$upby</td></tr>\n";
    }
    $csr -> finish;
    $outstr .= "</table>\n";
...
##################
    ...
    # 03/19/08 sh mod
   	if ($settings{rid} >= 30) { #rid:30 == revision 20 - sh 03/19/08
		    $outstr .= "<tr><td><b><li>NRC Document/Industry Standard:</b><br>\n";
		    $outstr .= "<textarea name=nrcdescription rows=5 cols=75>$nrcdoc</textarea></td></tr>\n";
		    $outstr .= "<tr><td><!--b><li>National/Industry Standard:</b><br-->\n";
		    $outstr .= "<input name=standarddescription type=hidden value='~$standard'></td></tr>\n";
   	}else{
   			$outstr .= "<tr><td><b><li>US NRC Document:</b><br>\n";
		    $outstr .= "<textarea name=nrcdescription rows=5 cols=75>$nrcdoc</textarea></td></tr>\n";
		    $outstr .= "<tr><td><b><li>National/Industry Standard:</b><br>\n";
    ...
    $outstr .= "<tr><td><b><li>OCRWM Position:&nbsp;&nbsp;</b><br><textarea rows=5 cols=75 name=position>$position</textarea></td></tr>\n";
...
#    $outstr .= "<tr><td align=center><input type=submit value=Submit onClick=javascript:submitFormCGIResults('qard','enter_table_process')></td></tr>\n";
    ...
    function validateStuff(command) {
        ...
        msg += (isblank(document.$args{form}.position.value)) ? "You must enter the OCRWM Position.\\n" : "";
        msg += (isblank(document.$args{form}.justification.value)) ? "You must enter the justification for the OCRWM Position.\\n" : "";
...
#########################
   ...
   #$outstr .= "<tr valign=top bgcolor=#f0f0f0><td width=40><font size=-1><b>Item</td><td><font size=-1><b>US NRC Document</td><td><font size=-1><b>National/Industry Standard</td><td><font size=-1><b>OCRWM Position</td>\n";
   # 03/19/08 sh mod
   	if ($settings{rid} >= 30) { #rid:30 == revision 20 - sh 03/19/08
   		$outstr .= "<tr valign=top bgcolor=#f0f0f0><td width=40><font size=-1><b>Item</td><td><font size=-1><b>NRC Document/Industry Standard</td><td><font size=-1><b>OCRWM Position</td>\n";
   	}else{
   		$outstr .= "<tr valign=top bgcolor=#f0f0f0><td width=40><font size=-1><b>Item</td><td><font size=-1><b>US NRC Document</td><td><font size=-1><b>National/Industry Standard</td><td><font size=-1><b>OCRWM Position</td>\n";
   	}
    ...
    for (my $i = 1; $i <= $#table; $i++) {
        ...
        # 03/19/08 sh mod
	   	if ($settings{rid} >= 30) { #rid:30 == revision 20 - sh 03/19/08
	   		$outstr .= "<tr valign=top><td><font size=2 face=Helvetica>$itemid</td><td width=150><font size=2 face=Helvetica>$nrcdoc</td><td><font size=2 face=Helvetica>$position</td>\n";
	   	}else{
	   		$outstr .= "<tr valign=top><td><font size=2 face=Helvetica>$itemid</td><td width=150><font size=2 face=Helvetica>$nrcdoc</td><td width=150><font size=2 face=Helvetica>$standard</td><td><font size=2 face=Helvetica>$position</td>\n";
	   	}
