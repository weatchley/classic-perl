# UI User functions
#
# $Source: /data/dev/rcs/prp/perl/RCS/UIUsers.pm,v $
# $Revision: 1.7 $
# $Date: 2005/10/06 16:33:30 $
# $Author: naydenoa $
# $Locker:  $
#
# $Log: UIUsers.pm,v $
# Revision 1.7  2005/10/06 16:33:30  naydenoa
# CREQ00065 - added CM Editor/Approver and WG member to user browse
#
# Revision 1.6  2005/09/28 23:39:55  naydenoa
# Phase 3 implementation
# Tweaked user display function to accommodate admin display at login
#
# Revision 1.5  2005/04/08 22:56:00  naydenoa
# Tweak to accommodate browse of QA Docs Editors - CREQ00047
#
# Revision 1.4  2005/04/07 19:35:17  naydenoa
# Added QA docs privileges to user display on browse - CREQ00047
#
# Revision 1.3  2004/06/16 21:27:29  naydenoa
# Updated privilege checks
#
# Revision 1.2  2004/05/07 21:58:46  naydenoa
# Added password security updates
# Affected functions: doChangePassword, doChangePasswordForm, doDisplayUser
#
# Revision 1.1  2004/04/22 20:43:28  naydenoa
# Initial revision
#
#
...
##############
    ...
    elsif (($args{command} eq "browse") || ($args{command} eq "displayuserform")) {
        $title = "Browse Users";
    } 
    elsif ($args{command} eq "displayuser") {
...
####################
    ...
    my $extracolumn = $args{from} ? &addCol (value => "Phone Number", width => 100) : "";
    
    my $output = "<a name='$args{title}'></a>\n";
    $output .= &startTable(columns => $args{columns}, align => $args{align}, title => $args{title});
    $output .= &startRow(bgColor => "#f0f0f0") . &addCol(value => "Name", width => 140) . &addCol(value => "Username", width => 100) . &addCol(value => "Organization", width => 120) . $extracolumn . &endRow;
    my @users = &getUserArray(dbh => $args{dbh}, schema => $args{schema}, startID => $args{startID}, endID => $args{endID}, privilege => $args{privilege});
    for (my $i=0; $i < $#users; $i++) {
        my ($id, $username, $lastname, $firstname, $organization, $areacode, $phonenumber) = ($users[$i]{id},$users[$i]{username},$users[$i]{lastname},$users[$i]{firstname},$users[$i]{organization}, $users[$i]{areacode}, $users[$i]{phonenumber});
        my $fullName = &get_fullname($args{dbh}, $args{schema}, $id);
        my $prompt = ($args{from}) ? "" : "Click here to display information for $fullName";
        my $url = ($args{from}) ? "" : "javascript:displayUser($id)";
        $output .= &startRow . &addCol(value => $fullName, url => $url, prompt => $prompt);
        my $extracontent = ($args{from}) ? &addCol (value => ($areacode . '-' . substr($phonenumber,0,3) . '-' . substr($phonenumber,-4,4))) : "";
        $output .= &addCol(value => $username) . &addCol(value => ((defined($organization)) ? $organization : "&nbsp;")) . $extracontent . &endRow;
    }
    $output .= &endTable;
...
##############
    ...
    $output .= "<center><table cellspacing=5 cellpadding=5 border=0><tr><td valign=top><b>";
    $output .= "<a href='#By Last Name' title='Click here to view by last name'>By Last Name</a>\n";
    $output .= "<br>\n";
    $output .= "<a href='#QARD Editors' title='Click here to view QARD editors'>QARD Editors</a>";
    $output .= "<br>\n";
    $output .= "<a href='#AQAP and QAMP Editors' title='Click here to view source documents editors'>AQAP and QAMP Editors</a>\n";
    $output .= "</td>";
    $output .= "<td><li>";
    $output .= "</td>";
    $output .= "<td valign=top><b>";
    $output .= "<a href='#Source Documents Editors' title='Click here to view source documents editors'>Source Documents Editors</a>";
    $output .= "<br>\n";
    $output .= "<a href='#Compliance Matrices Editors' title='Click here to view compliance matrices editors'>Compliance Matrices Editors</a>\n";
    $output .= "<br>\n";
    $output .= "<a href='#Compliance Matrices Approvers' title='Click here to view compliance matrices approvers'>Compliance Matrices Approvers</a>\n";
    $output .= "</td>";
    $output .= "<td><li>";
    $output .= "</td>";
    $output .= "<td valign=top><b>";
    $output .= "<a href='#System Administrators' title='Click here to view system administrators'>System Administrators</a>\n";
    $output .= "<br>\n";
    $output .= "<a href='#Working Group Members' title='Click here to view Working Group Members'>Working Group Members</a>\n";
    $output .= "<br>\n";
    $output .= "<a href='#Software Developers' title='Click here to view software developers'>Software Developers</a></b>\n";
    $output .= "</td>";
    $output .= "</tr></table><br>\n";
    $output .= &writeUserTable(dbh => $args{dbh}, schema => $args{schema}, endID => 9999, title => 'By Last Name');
    $output .= &writeUserTable(dbh => $args{dbh}, schema => $args{schema}, privilege => 3, title => 'QARD Editors', titleBackground => '#cdecff');
    $output .= &writeUserTable(dbh => $args{dbh}, schema => $args{schema}, privilege => 5, title => 'AQAP and QAMP Editors', titleBackground => '#cdecff');
    $output .= &writeUserTable(dbh => $args{dbh}, schema => $args{schema}, privilege => 4, title => 'Source Documents Editors', titleBackground => '#cdecff');
    $output .= &writeUserTable(dbh => $args{dbh}, schema => $args{schema}, privilege => 7, title => 'Compliance Matrices Editors', titleBackground => '#cdecff');
    $output .= &writeUserTable(dbh => $args{dbh}, schema => $args{schema}, privilege => 6, title => 'Compliance Matrices Approvers', titleBackground => '#cdecff');
    $output .= &writeUserTable(dbh => $args{dbh}, schema => $args{schema}, privilege => 10, title => 'System Administrators', titleBackground => '#cdecff');
    $output .= &writeUserTable(dbh => $args{dbh}, schema => $args{schema}, privilege => 2, title => 'Working Group Members', titleBackground => '#cdecff');
...
###################
    ...
    $uservalues[7][1] ='<b>' . $userInfo{sccbid} . '</b>';
...
#################
    ...
    if ($args{type} eq 'new') {
    $output .= "   submitForm('$args{form}','adduserform');\n";
}
else {
...
##########################
    ...
    my %settings = %$hashRef;
    my $output = "";
    my $passwordGuidelines = "<br><b>Password Guidelines:</b><br>\n";
    $passwordGuidelines .= "<li> must contain 8 or more characters</li>\n";
    $passwordGuidelines .= "<li> cannot contain form of username (forward or reverse)</li>\n";
    $passwordGuidelines .= "<li> cannot use some common strings</li>\n";
    $passwordGuidelines .= "<li> cannot start or end with a digit</li>\n";
    $passwordGuidelines .= "<li> must contain three of the following four:<br>\n";
    $passwordGuidelines .= &nbspaces(10) . "- a special character<br>\n";
    $passwordGuidelines .= &nbspaces(10) . "- a digit<br>\n";
    $passwordGuidelines .= &nbspaces(10) . "- an uppercase letter<br>\n";
    $passwordGuidelines .= &nbspaces(10) . "- a lowercase letter</li>\n";
    $passwordGuidelines .= "<li> cannot reuse the last six passwords</li>";
...
######################
    ...
    if ($status == 1) {
        ...
        my $alerttext = "Password does not meet security requirements";
        $alerttext .= " - you have used this password before" if ($reusedPassword);
        $output .= doAlertBox(text => $alerttext);
    } 
    else {
...
#####################
sub doResetPassword {  # routine to reset a user's password to system default
#####################
    my %args = (
        userID => 0,
        type => "Enable",
...
##########################
sub doBecomeUsernameForm { # routine to display form for becoming another user
##########################
    my %args = (
        userID => 0,
        type => "Enable",
...
######################
    ...
    my $userpriv = &doesUserHavePriv (dbh => $args{dbh}, schema => $args{schema}, userid => $userID, privList => [3, 4]);
    ...
    $output .= ($userpriv) ? "    submitForm('home','');\n" : "    submitForm('browse','');\n";
