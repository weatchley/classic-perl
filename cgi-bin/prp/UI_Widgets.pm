...
#
# $Source: /data/dev/rcs/prp/perl/RCS/UI_Widgets.pm,v $
# $Revision: 1.5 $
# $Date: 2005/10/06 16:34:29 $
# $Author: naydenoa $
# $Locker:  $
#
# $Log: UI_Widgets.pm,v $
# Revision 1.5  2005/10/06 16:34:29  naydenoa
# CREQ00065 - added options to drop-down functions
#
# Revision 1.4  2005/09/29 15:15:01  naydenoa
# Phase 3 implementation
# Added utilities to generate drop-downs for various data types
#
# Revision 1.3  2005/04/28 16:32:57  naydenoa
# Added back sub get_table - CREQ00053 - defect fix
#
# Revision 1.2  2005/04/15 20:42:59  naydenoa
# CREQ00052 - updated end year for date dropdowns to 2010
# Deleted some unused/superceded widgets
#
# Revision 1.1  2004/04/22 20:44:51  naydenoa
# Initial revision
#
#
...
@EXPORT = qw (
   ...
   &doAlertBox            &buildSectionBlock           &buildDateSelection
   &gen_table
   &makeQARDDropdown      &makeSourceDropdown          &makeMatrixDropdown
);
@EXPORT_OK = qw(
   &build_dual_select     &build_select_from_piclist   &build_drop_box 
   &nbspaces              &lpadzero                    &getDisplayString
...
###  'build_dual_select'  ###
#    (output string) = 
#         &build_dual_select( (element name), (form name), 
#                            \(hash of available values/names), 
#                            \(hash of selected values/names), 
#                             (name of available box (left side)), 
#                             (name of selected box (right side)) 
#                             [, (locked value)[, (locked value)
#                             [, (locked value) [...]]]] );
###  'build_select_from_piclist'  ###
#    (output string) = 
#         &build_select_from_piclist( (element name), (form name), 
#                                    \(array of values for piclist), 
#                                     (width of text), (size of piclist), 
#                                     (use a button: yes/true or no/false), 
#                                     (sort list: yes/true or no/false), 
#                                     (save data: yes/true or no/false), 
#                                     (max width), (default value) );
###  'build_drop_box'  ###
#    (output string) = 
#         &build_drop_box( (element name), \(hash of values/names),
#                          (selected value) );
###  'nbspaces'  ###
...
###  'getDisplayString'
#    (output string) = 
#         &getDisplayString( (input string), (max length of output string) );
###  'writeTitleBar'  ###
#    (output string) = 
#         &writeTitleBar( (username), (userid), (dbh), (schema), (title) );
###  'formatID'
#    (output string) = 
#        &formatID( (prepend string), (numeric length), (number) );
###  'doRemarks'
#    (output string) = 
#        &doRemarks( (arg hash - see function header documentation) );
###  'errorMessage'
#    (output string) = &errorMessage( (username), (userid), (schema), 
#                                   (application error), (oracle error) );
###  'doAlertBox'
#    (output string) = &doAlertBox( text => (text for alert) 
#                                    [, includeScriptTags => (T or F) ] );
###  'buildSectionBlock'
#    (output string) = 
#         &buildSectionBlock( contents => (contents of section), 
#                             title => (title of section) 
#                             [, button => (button style), 
#                             style => (section style), isOpen => (T or F) ] );
#

#######################
sub build_dual_select {  # build dual selection box from hashes passed to sub
#######################
    my $elementname=$_[0];
    my $formname = $_[1];
    my $availref = $_[2];
    ...
    my $outstring = '';
    my $username;
# remove entries from left table that exist in right table
...
###############################
sub build_select_from_piclist { # build combination text box and piclist 
###############################
    my $elementname=$_[0];
    my $formname = $_[1];
    my $piclistref = $_[2];
...
####################
sub build_drop_box { # routine to build a dropdown box from a passed hash
####################
    my $elementname=$_[0];
    my $valuesref = $_[1];
    my $selectedval=$_[2];
...
########################
sub buildDateSelection { # routine to build a date selection widget
########################
    my %args = (
        element => '',
        form => '',
    ...
    my $elementname = $args{element};
    my $formname = $args{form};
    my $initdate = ((defined($args{initdate})) ? $args{initdate} : ' ');
    my $startyear = $args{startyear};
    my $endyear = $args{endyear};
    my $outstr = '';
    ...
    $outstr .= "<select name=$elementname\_month size=1><option value=\"\"><option value=1>January<option value=2>February<option value=3>March<option value=4>April<option value=5>May<option value=6>June<option value=7>July<option value=8>August<option value=9>September<option value=10>October<option value=11>November<option value=12>December</select>\n";
    $outstr .= "<select name=$elementname\_day size=1><option value=\"\">\n";
    for (my $d = 1; $d <= 31; $d++) {
         $outstr .= "<option value=$d>$d\n";
    }
    $outstr .= "</select>,\n";
    $outstr .= "<select name=$elementname\_year size=1><option value=\"\">\n";
    for (my $i = $startyear; $i <= $endyear; $i++) {
        $outstr .= "<option value=$i>$i\n";
    }
    $outstr .= "</select>\n";
    $outstr .= "<script language=javascript><!--\n";
    $outstr .= "   set_selected_option(document.$formname.$elementname\_month, $month);\n";
    $outstr .= "   set_selected_option(document.$formname.$elementname\_day, $day);\n";
    $outstr .= "   set_selected_option(document.$formname.$elementname\_year, $year);\n";
    $outstr .= "//--></script>\n";
    return ($outstr);
}

##############
sub nbspaces { # routine to generate a string of non breaking spaces
##############
    my $numbofspaces = $_[0];
    my $outstring = "";
...
###############
sub gen_table { # generate standard two column table with a title cell
###############
    my $arrayref=$_[0];
    my $outstring = "";
    my $color;
...
######################
    ...
    my $rid = $args{rid} ? $args{rid} : 0;

    $outstr .= "<tr><td>\n" if $args{tablerow};
    $outstr .= "<select name=" . $args{qardtype} . "rid>\n";
    my $qardtype = ($args{qardtype}) ? $args{qardtype} : "qard";
    if ($args{qardtypeid} == 1) {
        $qardtype = "QARD";
    }
    elsif ($args{qardtypeid} == 2) {
        $qardtype = "AQAP";
    }
    $outstr .= "<option value=''>*** Select " . uc($qardtype) . " Revision ***\n" if $args{blankentry};
    $qardidlist = "(" if $args{idlist};
    my @qards = getQARDRevs (dbh => $args{dbh}, schema => $args{schema}, where => $args{where}, orderby => $args{orderby});
    for (my $i = 1; $i <= $#qards; $i++) {
        my $id = $qards[$i]{id};
        $qardidlist .= "$qards[$i]{id}, " if ($qards[$i]{iscurrent} eq 'L' && $args{idlist});
        my $revid = $qards[$i]{revid};
        my $status = $qards[$i]{iscurrent};
        if ($args{separate} && ($status ne $oldstatus)) {
            $outstr .= "<option value=''>____________________________\n";
            my $thestatus = ($status eq 'L') ? "Linked " : "Image-only ";
            $thestatus .= uc($qardtype) . " Revision(s)";
            $outstr .= "<option value=''>$thestatus\n";
            $outstr .= "<option value=''>-------------------------------------------------\n";
        }
        $oldstatus = $status;
        $selected = ($id == $rid) ? " selected" : "";
        $thesection = &getDisplayString ($revid, 49);
        $outstr .= "<option value=$id$selected>$thesection\n";
        $count++;
    }
    if ($args{idlist}) {
        chop ($qardidlist);
    ...
    return ($outstr, $qardidlist);
}

########################
sub makeSourceDropdown {
########################
    my %args = (
        where => '',
        orderby => '',
    ...
    $outstr .= "<tr><td>\n" if $args{tablerow};
    $outstr .= "<select name=" . $args{qardtype} . "sourceid>\n";
    $outstr .= "<option value=''>*** Select Source Document ***\n" if $args{blankentry};
    my @sources = getSourceDocs (dbh => $args{dbh}, schema => $args{schema}, where => $args{where}, orderby => $args{orderby});
    for (my $i = 1; $i <= $#sources; $i++) {
        my $id = $sources[$i]{id};
        my $designation = $sources[$i]{designation};
        my $title = $args{title} ? " $sources[$i]{title}" : "";
        $selected = ($args{sid} == $id) ? " selected" : "";
        $thesection = "$designation$title";
        $thesection = &getDisplayString ($thesection, 49);
        $outstr .= "<option value=$id$selected>$thesection\n";
    }
    $outstr .= "</select>\n";
    $outstr .= "</td></tr>\n" if $args{tablerow};
...
########################
    ...
    my $thesection = "";
    my $selected = "";
    my $matrixid = $args{matrixid} ? $args{matrixid} : 0;
    my $previd = 0;

    $outstr .= "<tr><td><select name=" . $args{qardtype} . "matrixid>\n";
    $outstr .= "<option value=''>*** Select Compliance Matrix$args{header} ***\n";
    $thesection = "";
    my @aqapmats = getMatrices (dbh => $args{dbh}, schema => $args{schema}, where => $args{where}, orderby => $args{orderby});
    for (my $i = 1; $i <= $#aqapmats; $i++) {
        my $id = $aqapmats[$i]{id};
        my $title = $aqapmats[$i]{title};
        $selected = ($matrixid == $id) ? " selected" : "";
        my $thetitle = &getDisplayString ($title, 49);
        $outstr .= "<option value=$id$selected>$thetitle\n" if ($id != $previd);
        $previd = $id
    }
    $outstr .= "</select>\n";
    $outstr .= "</td></tr>\n";
