...
# $Revision: 1.10 $
# $Date: 2006/06/13 23:40:09 $
# $Author: naydenoa $
# $Locker:  $
#
# $Log: qard.pl,v $
# Revision 1.10  2006/06/13 23:40:09  naydenoa
...
#############################################
    ...
    my $newcriteria = "";
    eval {
        ($name, $newcriteria, $oldcriteria) = &processMatrixEntryStoQ (dbh => $dbh, schema => $schema, title => $title, userID => $userid, form => $form, settings => \%settings);
    };
    if ($@) {
        ...
        my $activity = 32;
        $logstr = "Matrix $name information updated. $oldcriteria $newcriteria";
        &updateActivityLog ($dbh, $schema, $userid, $logstr, "F", $activity);
	print "<script language=javascript><!--\n";
	print "   alert(\"$logstr\");\n";
	print "   submitForm('home','');\n";
