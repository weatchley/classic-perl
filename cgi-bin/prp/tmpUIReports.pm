# UI Report functions
#
# $Source: /data/dev/rcs/prp/perl/RCS/UIReports.pm,v $
# $Revision: 1.28 $
# $Date: 2005/11/14 20:43:19 $
# $Author: naydenoa $
# $Locker:  $
#
# $Log: UIReports.pm,v $
...
#########################
    ...
    if ($args{qtos}) {
        ...
        for (my $i = 1; $i <= $#sourcesections; $i++) {
            ...
            my $qards = $args{dbh} -> prepare ("select s.id, s.sectionid, s.text, s.title, m.lastupdated, s.subid from $args{schema}.qardsection s, $args{schema}.qardmatrix m where m.matrixid = $matrixid and m.sourcerequirementid = $sid and s.isdeleted = 'F' and m.qardsectionid = s.id order by s.tocid, s.sorter, s.sectionid");
            $qards -> execute;
            my $theqards = "";
            while (my ($qid, $qsection, $qtext, $qtitle, $lastup, $qsubid) = $qards -> fetchrow_array) {
                my $fontcolor = ($args{approval} && ($lastup lt $args{dateapproved})) ? "<font color=#ff0000>" : "";
                $qtitle = ($qtitle ne "" && $qtitle ne " " && $qtitle) ? "<b>$qtitle</b><br><br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;" : "";
                $qsubid = ($qsubid) ? " - $qsubid" : " - 0";
                $theqards .= "$fontcolor$qsection$qsubid&nbsp;&nbsp;$qtitle$qtext<br><br>\n";
            }
            $qards -> finish;
...
########################
    ...
    my ($matrixtitle, $sourcedocid) = &getSingleRow (dbh => $args{dbh}, schema => $args{schema}, what => "title, sourceid", where => "id = $args{matrixid}", table => "matrix");
...
$^W = $warn;
        ...
        $qardsection = "";
        my $qards = $args{dbh} -> prepare ("select s.id, s.sectionid, s.subid, s.text, s.title from $args{schema}.qardsection s, $args{schema}.qardmatrix m where m.matrixid = $settings{matrixid} and m.sourcerequirementid = $docsectionid and s.isdeleted = 'F' and m.qardsectionid = s.id order by s.tocid, s.sorter, s.sectionid");
        $qards -> execute;
...
#    my %settings = %$hashRef;
    ...
    $outstr .= "Approval rejection rationale: $rationale" if ($status eq "Approval Pending");
    $outstr .= "</font>\n";
    $outstr .= "</td></tr></table>\n";
